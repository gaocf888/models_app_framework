...
async def classify_chatbot_intent_by_llm(
    ...
    *,
    ...
    try:
        ...
        logger.info(
            "chatbot.intent_llm narrow_trigger trigger=%s rule=%s/%s -> llm=%s conf=%.3f",
            trigger,
            ruled.intent_label,
            ruled.intent_reason,
            label,
        ...
        if cfg.intent_llm_fallback_to_rules:
            logger.warning(
                "chatbot.intent_llm failed, fallback rules trigger=%s rule=%s err=%s",
                trigger,
                ruled.intent_reason,
                e,
            )
