    ...
    return [
        ...
        SynthesisV2Slot(
            ...
            narrative_instruction=(
                "撰写本章正文（禁止输出「超温风险评估」、「风险评估正文」及任何 # / ## / ### 等标题行；）。"
                "禁止输出 docx 红色说明句「关联本次超温出现的问题，拉取知识片段并推理评估」。"
                "该章节仅需生成 超温风险评估 的正文，可基于相关数据，再衔接管材蠕变、氧化皮、金相劣化、爆管泄漏等风险机理，先按照区域分析（只输出分析结果，禁止输出相关测点超温明细数据），最后总结风险评估。"
        ...
        SynthesisV2Slot(
            ...
            source_item_ids=("q1", "q2", "q4", "q5"),
            narrative_instruction=(
                "撰写「紧急处置措施」正文（禁止输出「紧急处置」「紧急处置措施」及任何 # / ## / ### / #### 标题行；"
                "标题已由静态槽 s10a_emergency_hdr 输出）。"
                "【输出形态·必须遵守】仅用 1～2 段连贯中文段落总结，总字数 180～320 字；"
                "禁止分条列表、禁止 Markdown 小标题、禁止「首先/其次/再次」式多段结构。"
                "第 1 段用 2～4 句概括本次需立即关注的运行处置要点（监视、燃烧/负荷/减温水/主汽参数调整等），"
                "合并表述，不得按「整体调控、差异化降温、燃烧工况」拆成多个小节。"
                "仅当存在Ⅲ/Ⅳ级或连续超温时长明显偏长时，才允许第 2 段用 1～2 句写加强处置或升级关注；"
                "Ⅰ/Ⅱ级轻微超温禁止写检修动员、禁止写「持续恶化则联系检修」类套话。"
                "【内容取舍·必须遵守】区域/测点：最多点名 1 个最严重区域 + 1～2 个代表测点（测点名称（测点编号））；"
                "禁止逐条罗列 q1 中多个测点。"
                "数值：每段最多引用 3 个关键数（如最大超温值、连续时长、负荷或主汽压力）；其余用「等重点测点/工况」概括。"
                "须结合 q4 工况与 q1/q2 等级判断处置力度；无 q5 减温水/氧量等数据时禁止写具体调节量，写「待补充」或只写原则性监视。"
                "对Ⅲ/Ⅳ级且已知规格材质的测点/区域，处置须考虑材质耐温极限（如加强监视、限制升负荷）；无材质不写具体钢号。"
                "禁止重复第三章风险机理、禁止与「运行优化调整」（下一小节 s10b）相同的长期优化内容"
                "（如吹灰策略、检修预防、氧化皮检测计划）。"
                "【通用禁令】禁止输出 docx 示例措施全文；禁止置信度、依据、q 编号；"
                "禁止「可能/或许/大概/一般/通常」等泛化表述；无数据支撑的操作写「待补充」；禁止编造已执行操作。"
            ),
        ),
        SynthesisV2Slot(
        ...
        SynthesisV2Slot(
            ...
            source_item_ids=("q1", "q2", "q4", "q5"),
            narrative_instruction=(
                "撰写「运行优化调整」正文（禁止输出「运行优化调整」及任何 # / ## / ### / #### 标题行；"
                "标题已由静态槽 s10b_optimize_hdr 输出）。"
                "【输出形态·必须遵守】仅用 1～2 段连贯中文段落总结，总字数 180～320 字；"
                "禁止分条列表、禁止 Markdown 小标题、禁止「首先/其次/再次」式多段结构。"
                "第 1 段用 2～4 句概括运行侧可执行的优化方向（燃烧配风、负荷分配、减温水调节、吹灰策略、主汽参数优化等），"
                "合并表述，不得按「共性优化、专项优化」拆成多个小节。"
                "仅当存在多区域差异或Ⅲ/Ⅳ级持续超温时，才允许第 2 段用 1～2 句写针对性加强优化；"
                "Ⅰ/Ⅱ级轻微超温禁止写大修级或检修动员类内容。"
                "【内容取舍·必须遵守】区域/测点：最多点名 1～2 个重点超温区域 + 1～2 个代表测点（测点名称（测点编号））；"
                "禁止逐条罗列 q1 中多个测点。"
                "数值：每段最多引用 3 个关键数（如超温极值、连续时长、负荷或主汽压力）；其余用「等重点区域/工况」概括。"
                "优化建议须结合 q4 工况与 q1/q2 等级；须对应区域规格材质特性（如高合金氧化皮风险区避免频繁启停变温）；"
                "无 q5 减温水/氧量等数据时禁止写具体调节量，写「待补充」或只写原则性优化方向。"
                "禁止重复「紧急处置措施」（上一小节 s10a）的立即监视与紧急操作内容；"
                "禁止与「后续检修预防措施」（s10c）、「计划长效防控方案」（s10d）相同的检修计划或长效管控内容。"
                "【通用禁令】禁止输出 docx 示例措施全文；禁止置信度、依据、q 编号；"
                "禁止正文单独以测点编号或测点编码作主称谓；"
                "禁止「可能/或许/大概/一般/通常」等泛化表述；无依据写「待补充」；禁止编造已执行操作。"
            ),
        ),
        SynthesisV2Slot(
        ...
        SynthesisV2Slot(
            ...
            source_item_ids=("q1", "q2", "q4", "q5"),
            narrative_instruction=(
                "撰写「后续检修预防措施」正文（禁止输出「后续检修预防措施」及任何 # / ## / ### / #### 标题行；"
                "标题已由静态槽 s10c_maintenance_hdr 输出）。"
                "【输出形态·必须遵守】仅用 1～2 段连贯中文段落总结，总字数 180～320 字；"
                "禁止分条列表、禁止 Markdown 小标题、禁止「首先/其次/再次」式多段结构。"
                "第 1 段用 2～4 句概括检修与预防重点（重点部位检查、氧化皮/蠕变评估、测点校验、换管或材质升级等），"
                "合并表述，不得按「重点检修、全面排查、测点校验」拆成多个小节。"
                "仅当存在Ⅲ/Ⅳ级或重复超温区域时，才允许第 2 段用 1～2 句写加强排查或专项检测；"
                "Ⅰ/Ⅱ级轻微超温禁止写大规模停炉检修或换管计划，可写加强监视与例行检查。"
                "【内容取舍·必须遵守】区域/测点：最多点名 1～2 个严重超温区域 + 1～2 个代表测点（测点名称（测点编号））；"
                "禁止逐条罗列 q1 中多个测点。"
                "须引用 q1 区域名称、规格材质与等级写针对性检修建议；无材质写「规格材质待补充」。"
                "数值：每段最多引用 3 个关键数（如超温极值、连续时长、等级）；其余用「等重点区域」概括。"
                "禁止重复「紧急处置措施」（s10a）的立即操作与「运行优化调整」（s10b）的日常优化内容；"
                "禁止与「计划长效防控方案」（s10d）相同的制度性、长效管控方案。"
                "【通用禁令】禁止输出 docx 示例措施全文；禁止置信度、依据、q 编号；"
                "禁止正文单独以测点编号作主称谓；"
                "禁止「可能/或许/大概/一般/通常」等泛化表述；无依据写「待补充」；禁止编造已执行操作。"
            ),
        ...
        SynthesisV2Slot(
            ...
            source_item_ids=("q1", "q2", "q4", "q5"),
            narrative_instruction=(
                "撰写「计划长效防控方案」正文（禁止输出「计划长效防控方案」及任何 # / ## / ### / #### 标题行；"
                "标题已由静态槽 s10d_longterm_hdr 输出）。"
                "【输出形态·必须遵守】仅用 1～2 段连贯中文段落总结，总字数 180～320 字；"
                "禁止分条列表、禁止 Markdown 小标题、禁止「首先/其次/再次」式多段结构。"
                "第 1 段用 2～4 句概括长效防控思路（分区管控、重复超温区域重点监控、运行标准优化、"
                "定期评估与预警机制等），合并表述，不得按「优化措施、分区管控」拆成多个小节。"
                "仅当存在多区域重复超温或历史频发特征时，才允许第 2 段用 1～2 句写制度化或周期化防控安排；"
                "单次轻微超温禁止写空泛的长期规划套话。"
                "【内容取舍·必须遵守】须结合本次超温分布与重复发生区域，最多点名 1～2 个重点区域 + 1～2 个代表测点"
                "（测点名称（测点编号））；禁止逐条罗列 q1 中多个测点。"
                "数值：每段最多引用 3 个关键数（如超温频次、极值、连续时长）；其余用「等重点区域」概括。"
                "禁止重复「紧急处置措施」（s10a）、「运行优化调整」（s10b）、「后续检修预防措施」（s10c）"
                "中已述的立即操作、日常优化与单次检修内容；本节聚焦中长期制度与周期性防控。"
                "【通用禁令】禁止输出 docx 示例措施全文；禁止置信度、依据、q 编号；"
                "禁止正文单独以测点编号作主称谓；"
                "禁止「可能/或许/大概/一般/通常」等泛化表述；无依据写「待补充」；禁止编造已执行操作。"
            ),
