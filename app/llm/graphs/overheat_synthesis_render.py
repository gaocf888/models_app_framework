...
OVERHEAT_CH1_INTRO = (
    "## 超温情况概览\n\n"
    "没有询问具体锅炉时，需要输出所有锅炉内容。\n"
    "异常等级分为：Ⅰ级（轻微超温）、Ⅱ级（中度超温）、Ⅲ级（严重超温）、Ⅳ级（临界爆管）\n\n"
    "该章节数据和报告要求：\n"
    "Query 中如果是问今天/昨天/前天/等某一天，报告仅关联获取指定一天的数据和报告；\n"
    "Query 中如果是问本周/上周/上上周/本月/上月等，报告需关联获取指定日期内按周统计的数据和报告；\n"
    "Query 中如果是指定了机组，获取和实现指定机组数据和报告；若未指定机组，需要获取所有机组数据和报告。"
    "（查询 sql 首先按照机组进行排序，如果查询结果是多个机组数据，要分开表格展示每个机组的数据表格）\n\n"
)
