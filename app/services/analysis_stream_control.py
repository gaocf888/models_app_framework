...
class AnalysisStreamControl:
    """
    综合分析流式中断控制器（Redis 优先，内存回退）。

    语义与 ``ChatbotStreamControl`` 一致，键空间独立为 ``analysis:stream:*``。
    """
    ...
    def __init__(self, ttl_seconds: int = 900) -> None:
        ...
        if redis_url and aioredis is not None:
            try:
                ...
                logger.warning("AnalysisStreamControl redis init failed, fallback memory: %s", exc)
    ...
    async def cancel_stream(self, user_id: str, session_id: str, stream_id: str) -> None:
        if self._redis is not None:
            try:
                ...
                logger.warning("AnalysisStreamControl cancel redis failed, fallback memory: %s", exc)
    ...
    async def is_cancelled(self, user_id: str, session_id: str, stream_id: str) -> bool:
        if self._redis is not None:
            try:
                ...
                logger.warning("AnalysisStreamControl check redis failed, fallback memory: %s", exc)
        async with self._lock:
            now = time.time()
    ...
    async def clear_stream(self, user_id: str, session_id: str, stream_id: str) -> None:
        if self._redis is not None:
            try:
                ...
                logger.warning("AnalysisStreamControl clear redis failed, fallback memory: %s", exc)
        async with self._lock:
    ...
    async def _touch_stream_async(self, user_id: str, session_id: str, stream_id: str) -> None:
        ...
        try:
            ...
            logger.warning("AnalysisStreamControl touch redis failed: %s", exc)

    @staticmethod
    def _stop_key(user_id: str, session_id: str, stream_id: str) -> str:
        return f"analysis:stream:stop:{user_id}:{session_id}:{stream_id}"

    @staticmethod
    def _active_key(user_id: str, session_id: str, stream_id: str) -> str:
        return f"analysis:stream:active:{user_id}:{session_id}:{stream_id}"
