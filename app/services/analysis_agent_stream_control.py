...
"""综合分析智能体流式中断（键空间 analysis_agent:stream:*）。"""
...
class AnalysisAgentStreamControl:
    """语义对齐 AnalysisStreamControl / ChatbotStreamControl，独立键前缀。"""
    ...
    def __init__(self, ttl_seconds: int = 900) -> None:
        ...
        if redis_url and aioredis is not None:
            try:
                ...
                logger.warning("AnalysisAgentStreamControl redis init failed, fallback memory: %s", exc)
    ...
    async def cancel_stream(self, user_id: str, session_id: str, stream_id: str) -> None:
        if self._redis is not None:
            try:
                ...
                logger.warning("AnalysisAgentStreamControl cancel redis failed, fallback memory: %s", exc)
    ...
    async def is_cancelled(self, user_id: str, session_id: str, stream_id: str) -> bool:
        if self._redis is not None:
            try:
                ...
                logger.warning("AnalysisAgentStreamControl check redis failed, fallback memory: %s", exc)
        async with self._lock:
            now = time.time()
    ...
    async def clear_stream(self, user_id: str, session_id: str, stream_id: str) -> None:
        if self._redis is not None:
            try:
                ...
                logger.warning("AnalysisAgentStreamControl clear redis failed, fallback memory: %s", exc)
        async with self._lock:
    ...
    async def _touch_stream_async(self, user_id: str, session_id: str, stream_id: str) -> None:
        ...
        try:
            ...
            logger.warning("AnalysisAgentStreamControl touch redis failed: %s", exc)

    @staticmethod
    def _stop_key(user_id: str, session_id: str, stream_id: str) -> str:
        return f"analysis_agent:stream:stop:{user_id}:{session_id}:{stream_id}"

    @staticmethod
    def _active_key(user_id: str, session_id: str, stream_id: str) -> str:
        return f"analysis_agent:stream:active:{user_id}:{session_id}:{stream_id}"
