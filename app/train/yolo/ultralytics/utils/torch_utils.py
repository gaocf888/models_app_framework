...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    device_raw = str(device).lower()
    device = device_raw
    for remove in "cuda:", "mlu:", "none", "(", ")", "[", "]", "'", " ":
    ...
    mlu_available = bool(
        (hasattr(torch, "mlu") and hasattr(torch.mlu, "is_available") and torch.mlu.is_available())
        or (hasattr(torch, "is_mlu_available") and torch.is_mlu_available())
    )
    mlu_requested = device_raw.startswith("mlu")
    cuda_requested = device_raw.startswith("cuda")

    # Prefer MLU when available unless the user explicitly requests CUDA.
    mlu = (not cpu and not mps) and (mlu_requested or (mlu_available and not cuda_requested))

    def _parse_device_list(d: str) -> list[int]:
        return [int(x) for x in d.split(",") if x != ""]

    if cpu or mps:
        os.environ["CUDA_VISIBLE_DEVICES"] = "-1"  # force torch.cuda.is_available() = False
    elif mlu:  # MLU device requested
        if device == "mlu":
            device = "0"
        if "," in device:
            device = ",".join([x for x in device.split(",") if x])
        visible = os.environ.get("MLU_VISIBLE_DEVICES", None)
        ...
        mlu_count = int(torch.mlu.device_count()) if (mlu_ok and hasattr(torch.mlu, "device_count")) else 0
        ...
        if not mlu_ok:
            LOGGER.info(s)
            raise ValueError(
                f"MLU is not available but 'device={device}' was requested."
                f"\nos.environ['MLU_VISIBLE_DEVICES']: {visible}"
            )

        if mlu_count < n:
            LOGGER.info(s)
            raise ValueError(
                f"Invalid MLU 'device={device}' requested."
                f"\ntorch.mlu.device_count(): {mlu_count}"
                f"\nos.environ['MLU_VISIBLE_DEVICES']: {visible}\n"
                "MLU runtime reports fewer visible devices than requested. Verify MLU driver/NEUWARE version and device mapping."
            )

        if n > 1:
            if batch < 1:
                raise ValueError(
                    "AutoBatch with batch<1 not supported for Multi-MLU training, please specify a valid batch size."
                )
            if batch >= 0 and batch % n != 0:
                raise ValueError(f"'batch={batch}' must be a multiple of MLU count {n}.")

        space = " " * (len(s) + 1)
        for i, d in enumerate(devices):
            s += f"{'' if i == 0 else space}MLU:{d}\n"
        arg = "mlu:0"

    elif device and not mlu:  # non-cpu device requested (CUDA)
        if device == "cuda":
            device = "0"
        if "," in device:
        ...
        requested = _parse_device_list(device) if device else [0]
        cuda_ok = torch.cuda.is_available() and torch.cuda.device_count() > 0
        cuda_count = torch.cuda.device_count() if cuda_ok else 0
        indices_ok = (max(requested) < cuda_count) if cuda_ok else False

        if not (cuda_ok and indices_ok):
            LOGGER.info(s)
            install = (
    ...
    if not cpu and not mps and not mlu and torch.cuda.is_available():  # prefer GPU if available
