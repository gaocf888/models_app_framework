...
from app.small_models.strategy.face.models import FaceGallery, FacePerson, FaceSample, new_sample_id
...
def _milvus_sync_after_save(gallery: FaceGallery) -> None:
    if not use_milvus_backend():
        return
    from app.small_models.strategy.face.backends.milvus_index import milvus_sync_gallery
    ...
    milvus_sync_gallery(gallery.gallery_id, samples, names)


def _milvus_upsert_sample(gallery_id: str, sample: FaceSample, person_name: str) -> None:
    from app.small_models.strategy.face.backends.milvus_index import milvus_upsert_sample

    milvus_upsert_sample(gallery_id, sample, person_name=person_name)


def _milvus_delete_gallery(gallery_id: str) -> None:
    from app.small_models.strategy.face.backends.milvus_index import milvus_delete_samples

    milvus_delete_samples(gallery_id)


def _milvus_delete_person(gallery_id: str, person_id: str) -> None:
    from app.small_models.strategy.face.backends.milvus_index import milvus_delete_samples

    milvus_delete_samples(gallery_id, person_id=person_id)


class FaceGalleryStore:
    """人脸库持久化（JSON 元数据 + 图片）；向量检索可选 Milvus。"""
    ...
    def delete_gallery(self, gallery_id: str) -> bool:
        with self._lock:
            ...
            if use_milvus_backend():
                _milvus_delete_gallery(gallery_id)
    ...
    def delete_person(self, gallery_id: str, person_id: str) -> bool:
        with self._lock:
            ...
            if person_id not in gallery.persons:
                return False
            if use_milvus_backend():
                _milvus_delete_person(gallery_id, person_id)
    ...
    def enroll(
        ...
        *,
        ...
        with self._lock:
            ...
            self._save_gallery_unlocked(gallery)
            if use_milvus_backend():
                _milvus_upsert_sample(gallery_id, sample, person.name)
            return sample
    ...
    def get_gallery_index(self, gallery_id: str) -> Any:
        """检索索引：local 为内存 numpy/faiss；milvus 为远程向量检索。"""
        with self._lock:
            gallery = self._load_gallery_unlocked(gallery_id)
            if gallery is None:
                raise ValueError(f"gallery not found: {gallery_id}")
            ...
            if use_milvus_backend():
                samples, names = self._collect_samples_unlocked(gallery)
            ...
            samples, names = self._collect_samples_unlocked(gallery)
            index = build_gallery_index(gallery_id, samples, names, gallery.updated_at)
            _index_cache[gallery_id] = (gallery.updated_at, index)
            return index

    def _collect_samples_unlocked(self, gallery: FaceGallery) -> tuple[List[FaceSample], dict[str, str]]:
        samples: List[FaceSample] = []
        names: dict[str, str] = {}
        for person in gallery.persons.values():
            names[person.person_id] = person.name
    ...
    def gallery_index_stats(self, gallery_id: str) -> dict[str, Any]:
        ...
        if use_milvus_backend():
            stats["vector_store"] = "milvus"
        else:
            stats["vector_store"] = "local"
        return stats

    def sync_gallery_vectors(self, gallery_id: str) -> dict[str, Any]:
        """将 JSON 中样本全量同步到 Milvus（迁移/修复）。"""
        if not use_milvus_backend():
            raise RuntimeError("sync_gallery_vectors requires FACE_VECTOR_BACKEND=milvus")
        with self._lock:
            gallery = self._load_gallery_unlocked(gallery_id)
            if gallery is None:
                raise ValueError(f"gallery not found: {gallery_id}")
            _milvus_sync_after_save(gallery)
            samples, _ = self._collect_samples_unlocked(gallery)
            return {"gallery_id": gallery_id, "synced_samples": len(samples)}
