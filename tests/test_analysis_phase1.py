...
class TestAnalysisPhase1Runner(unittest.TestCase):
    def test_payload_mode_returns_structured_result(self):
        ...
        self.assertEqual("test_v1", result.trace.template_versions["synthesis"])
        self.assertEqual("test_v1", result.trace.template_versions["report"])
    ...
    def test_nl2sql_mode_records_calls(self):
        ...
        self.assertEqual(2, len(result.evidence.nl2sql_calls))
        self.assertEqual(2, result.evidence.data_coverage.get("success_calls"))
        ...
        self.assertIn("threshold_result", result.evidence.data_coverage["data_quality_report"])
        self.assertEqual("test_v1", result.trace.template_versions["intent"])
        ...
        self.assertGreaterEqual(len(result.evidence.rag_sources), 1)
        self.assertIn("graph_nodes", result.trace.execution_summary)
        self.assertEqual(2, nl2sql.query.await_count)
