...
| **In：开关** | 四类专项各自环境变量：`true` = 程序表 + LLM 叙述 + 拼接；`false` = **与现网完全相同**（整篇 LLM + `json.dumps[:4000]`）。 |
...
| 代码触点 | 主要为 `AnalysisGraphRunner` 的 `_build_summary_prompt`、`_generate_summary`、`_stream_summary_text`、`iter_nl2sql_stream_events`、`_finalize_nl2sql_sequential_v2`；新增独立表渲染模块，**不扩散到** `nl2sql/chain.py`。 |
...
| **P0** | R5 | 编排接入（同步） | `compose_nl2sql_summary`、`_build_summary_prompt` 分支、`_generate_summary` / `_finalize_nl2sql_sequential_v2` | R3、R4 |
...
| `synthesis` | `json.dumps(gathered_data)[:4000]` 注入 LLM | 全量 q0～qn 远超 4KB，模型几乎看不到 q1 明细 |
...
| 环境变量 | 说明 |
|----------|------|
| `ANALYSIS_LLM_DIGEST_MAX_CHARS` | 替代固定 4000，供 LLM 叙述章使用的 digest 字符上限（建议默认 8000） |
...
if deterministic_tables_enabled(analysis_type) and data_mode == "nl2sql":
    ...
    保持现逻辑：json.dumps(data_blob)[:4000]
```
...
|------|----------------------|------------------------|
| synthesis 数据输入 | `gathered_data` JSON 截断 4000 字符 | `build_llm_data_digest` 统计摘要 |
