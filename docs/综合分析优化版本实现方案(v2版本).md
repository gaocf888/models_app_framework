...
> 版本：2026-05 修订（2026-05-25 超温 q1～q6 / 14 槽 / 提示词分工落地）  
...
| **`analysis_plan_*` 版本选择** | **建议联动** | v2 策略下应固定或显式选用 **`analysis_plan_<type>` v2**（超温为 **q1～q6**），否则 `gathered_data` 与九章节模板不匹配 |
...
| 配置 / 代码 | v2 多槽 **运行时是否注入 LLM** | 作用 |
|-------------|-------------------------------|------|
| `analysis_synthesis_overheat_guidance` **v2** | **否** | 九段《锅炉管壁超温智能分析报告》**结构蓝本**（`prompts.yaml`）；**v1 策略**整篇合成时作 system |
| `analysis_synthesis_overheat_narrative` | **是（所有 LLM 槽 system）** | 全局写作硬约束 + q1～q6 数据映射说明 |
| `_overheat_v2_slots().narrative_instruction` | **是（各 LLM 槽 user·本章任务）** | 按章写作条目（与 v2 九段模板对齐，当前写在 `analysis_synthesis_v2.py`） |
| `analysis_plan_overheat_guidance` **v2** | **是（acquire_data）** | q1～q6 NL2SQL 自然语言计划；参考 SQL 见 `docs/综合分析-超温分析 报告模板及查询数据逻辑/overheat_plan_v2_reference_sql.sql` |

**LLM 槽单次调用消息结构：**

```text
system  ← analysis_synthesis_overheat_narrative
user    ← 用户问题 + planning_context(可选) + 【可引用事实】 + q切片JSON + RAG + 【本章写作任务】← narrative_instruction
```
...
| item_id | 报告章节 | 摘要 |
|---------|----------|------|
| q1 | 一、报告基础信息 | 锅炉台账 + 测点分级统计 |
| q2 | 二、超温事件概况 | 时段、工况、分级、极值 |
| q3 | 三、数据统计 | 区域汇总 + 尖峰频次 |
| q4 | 三、关联参数 | 壁温时序 + SIS 关联测点 |
| q5 | 七、缺陷与验证 | 遗留/泄漏/换管 + 效果验证 |
| q6 | 九、附件 | 趋势、对照、历史对标 |

**槽位（14 个，P0 超温）** — 详见 `enterprise-level_transformation_docs/系统整体技术实现-简版.md` §1.1.6 或 `app/llm/graphs/analysis_synthesis_v2.py` `_overheat_v2_slots()`。

槽位类型与生成方式：

| 槽位类型 | 典型章节 | 数据来源 | 生成方式 |
|----------|----------|----------|----------|
| `llm_narrative` | 一、二、三叙述、四～八 等 | 绑定 **q 切片** JSON + RAG + `narrative_instruction` | 每章独立 `chat`/`stream_chat`；system=`overheat_narrative` |
| `table_deterministic` | 三、七、九 数据表 | `gathered_data[q*]` | **程序渲染** Markdown 表 + `structured_report.tables[]` |
| `chart_structured` | 九、附件趋势 | `gathered_data[q6]` 等 | 程序生成 **图表 spec** + 可选 Markdown 说明 |
| `static_markdown` | 九、附件说明 | 无 | 固定文案 |

~~槽位定义建议放在 YAML 或代码注册表~~ → **当前已落地为代码注册表** `SYNTHESIS_V2_SLOT_REGISTRIES["overheat_guidance"]`（14 槽）；后续可外置 YAML。

```yaml
# 历史示例（未采用外置 YAML；现用 analysis_synthesis_v2.py 内 _overheat_v2_slots）
slots:
  - id: s01
    kind: llm_narrative
    source_item_ids: [q1]
    narrative_instruction: "一、报告基础信息 …"
```
...
| 项目 | v1 | v2 |
|------|----|----|
| 数据计划 | q1～q4 聚合（plan v1） | **q1～q6**（plan v2，对齐九段报告 + 附件） |
| synthesis system | `analysis_synthesis_overheat_guidance` 整篇 | **`analysis_synthesis_overheat_narrative`** + 各槽 `narrative_instruction` |
| synthesis 输出 | 单次 LLM，六章+附录表 | **14 槽**（8×LLM + 4×程序表/图 + 1×静态）；表程序出、叙述多段 LLM |
...
2. **plan v1/v2 与 gathered_data 键**：v2 策略必须配 **plan v2**（超温 **q1～q6**），否则槽位 `source_item_ids` 错位。  
...
### 阶段 B：超温 v2 核心（约 5～8 人日）— **已完成（2026-05-25）**

- 槽位注册表（**14 槽** → 表/图/LLM 段映射 **q1～q6**）。  
- 程序表 Markdown + `structured_report.tables`；chart 槽 + `charts`。  
- `analysis_synthesis_overheat_narrative` + 各槽 **`narrative_instruction`** + 有限并行 LLM。  
- **按槽位顺序串行** `iter_stream_events` → 对接 `summary_delta`；首槽 `stream_live`。  
- 单测：`tests/test_analysis_synthesis_v2.py`。
