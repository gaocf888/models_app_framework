...
确认解析出的 **锅炉名 + 设备名**（及可选管排）在账册表中 **真实存在**，避免「LLM 填了看似合理但库中无记录」的组合进入 NL2SQL。
...
1. **`piperow_name` 须 optional guard**：用户未提供管排时不能 `INNER JOIN + 等值`，否则几乎总是 0 行。
2. **`row_no` / `tube_no`**：账册表若无对应列，本层 **不参与校验**；排/管精度留给 NL2SQL plan SQL 与业务表。
3. **通过条件**：`record_count > 0` → 进入并行臂；`= 0` → interrupt，提示「范围在库中未找到，请核对机组/受热面/管排」。
4. **执行**：只读 COUNT；超时/连接失败 → 可配置为「跳过校验继续」（degrade）或 strict 失败。
