...
>
> 实现落点：`chatbot_intent_disambiguation.py`、`chatbot_hitl*.py`、`chatbot_graph_runner.py`；配置见 `CHATBOT_INTENT_DISAMBIGUATION_*` / `CHATBOT_INTENT_HITL_MAX_ROUNDS`。
...
`route_clarify` 续跑路径会先 `_node_intent_classify`，分类结果会覆盖相关状态，导致简单「防循环」短路在二次失败场景下 **实际未生效**。

因此当 `refined_query` 与原问句相同或仍模糊时：

- 仍可能再次满足 `should_trigger_intent_hitl`；
- 若再次中断，会 **重复返回** 同一组三按钮（`route_data_query` / `route_kb_qa` / `route_clarify`）；
- 用户体验差：已补充过问题，仍被要求做「路由方式」选择，而非更具体的问题消歧。

**落地后**：二次失败进入 `intent_disambiguation_suggest`（候选问法点选 + `route_hint` 直路由），不再重复三路由按钮。
...
- **prompt**：LLM 的 `analysis` + 固定引导语（请点击下方问题链接继续）；**不再**在 prompt 中罗列 1/2/3 问题列表（方案 A）。
- **ui_buttons**（动态生成，`label` = 完整 `query`），示例：

```json
[
  {"id": "pick_disambiguation_0", "label": "今天有多少超温数据？"},
  {"id": "pick_disambiguation_1", "label": "今天超温数据的原因是什么？"},
  {"id": "pick_disambiguation_2", "label": "今天超温相关标准与处理建议有哪些？"}
]
```

- **前端**：`hitl_kind=intent_disambiguation_suggest` 时将 `ui_buttons` 渲染为**链接**；其它 hitl_kind 仍为按钮。
- **context** 附带：`disambiguation_options`（含 `title` / `query` / `route_hint`）、`disambiguation_source`。

前端测试页（`tests/web/chatbot-stream.html`）已按上述分流渲染。注意：`DEFAULT_HITL_BUTTONS` 回落逻辑需区分新 `hitl_kind`，避免空按钮时错误回落到三路由。
...
```json
{
  ...
  "prompt": "您这个问题里，既像在查相关数据，也像在问原因或说明。我怕一次答偏了，想先跟您对齐一下想先解决哪一块。\n\n下面有几句更具体的问法，您点最贴近您意思的那一句就行，我按您选的继续答。",
  "ui_buttons": [
    {"id": "pick_disambiguation_0", "label": "今天有多少超温数据？"},
    {"id": "pick_disambiguation_1", "label": "今天超温数据的原因是什么？"},
    {"id": "pick_disambiguation_2", "label": "今天超温相关标准与处理建议有哪些？"}
...
| `CHATBOT_INTENT_DISAMBIGUATION_TIMEOUT_SEC` | `15` | LLM 消歧超时（实测约 8s，默认留余量） |
