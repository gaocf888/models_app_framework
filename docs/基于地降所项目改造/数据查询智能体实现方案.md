...
POST /data-query-agent/run-stream
  ...
  → acquire_data            ★ NL2SQL：【库(表)+区+站+时间】+ 可选锁表 forced_tables → 生成 SQL + 执行
                             列表（+ 站点维时 HUD 历史序列；两路 SQL 均带同一库表）
  → assemble_result         ★ hud_enabled + 列表 + hud_by_station
...
| `library_id` | 展示名 | 物理表 | 站点维 HUD 主序列 | 核心指标（HUD） |
|--------------|--------|--------|-------------------|-----------------|
...
| 岩性 / 优势层 | **无此列** | 本期不查、不展示；问句出现岩性 → warning，不 HITL |
...
| `options.include_hud` | 否 | 默认 `true`：站点维时附带 HUD 历史；`false` 仅列表（调试/导出） |
...
列表页与 HUD **不二次请求算法接口**。一次 `run-stream`（含 resume 续流）的终局 payload 同时包含：

- `list`：表格行  
- `hud_enabled`：前端是否渲染「加载 HUD」  
- `hud_by_station`：仅当 `hud_enabled=true` 时存在  

Java 默认列表/点树列表若也要 HUD，由 Java 自己组同等结构，或后续 P2 提供「按 station_id 拉 HUD」的轻量接口；**P0 算法只保证解析 SQL 这一路。**

### 7.2 `hud_enabled` 判定（必须机读）

仅当列表是 **监测站点维**（一行 ≈ 一个监测站的最新/窗口代表值）时为 `true`。

| 查询形态 | `result_grain` | `hud_enabled` |
|----------|----------------|---------------|
| 各站最新/窗口沉降、水位、位移等 | `station` | `true` |
| 按行政区聚合、全市汇总、只有区无站 | `district` / `city` | `false` |
| 纯时序单站但问的是「一条曲线」且列表不是多站 | `station_series` | `true`（一行站 + 序列仍给 HUD） |
| 无法判定 | `unknown` | `false`（保守，避免错误 HUD 按钮） |

判定顺序：

1. **计划侧（主）：** 取数模板按意图选择 `q_list` 形态（见 §7.3）。站点列表模板 ⇒ 预置 `station`。  
2. **结果侧（校验）：** 行集合存在 `station_id`（或 `station_name`）且未以 `area` 为唯一分组键。  
3. 注册表 `hud_supported`：七库 P0 均为 `true`；若未来增加非站点表可关掉。

### 7.3 取数：两个 NL2SQL 子任务（非报告 plan）

不要套 `analysis_plan_subsidence_*`。本模块内置 **极小数据计划**（可放 `configs/data_query_agent/plan.yaml`）：

| `plan_item_id` | 何时执行 | 目的 |
|----------------|----------|------|
| `q_list` | 每次 | 列表：站点维时「每站一行 + 核心指标」；非站点维时聚合表 |
| `q_hud_series` | 仅 `hud_enabled` 且 `options.include_hud` | 列表内各站的历史轨迹（折线） |

同层无依赖，可并行（`DATA_QUERY_AGENT_ACQUIRE_MAX_PARALLEL`，默认 2）。

两路任务的 SQL **都必须**带上已锁定的库表；区/站/时间作为同一 WHERE 来源（§5.3）。

**`q_list` 站点维问句约束（程序生成，喂 NL2SQL）：**

- **主表 = 当前 `library_id` 对应表**（唯一 FROM 事实表）；JOIN `t_station` 取 `area`。  
- 每站最新 `data_time`（或时间窗内末日）一条。  
- 核心度量列按 **该库** 映射（§2），不得用其它库的列名顶替（例如 GNSS 禁用 `total_settle` 冒充沉降）。  
- 年沉降/年度变化：窗 = 意图 2 的年窗，否则近 365 天；`end_value - start_value`。  
- `ORDER BY` 核心指标绝对值或 `station_id`；`LIMIT` = `max_rows`。

**`q_hud_series`：**

- **同一库表** 上取时序，禁止改查另一张 `t_data_wash_*`。  
- 过滤 `station_id IN (q_list 的站集合)`；若并行则用同一套区/站/库条件拉序列，assemble 时按 list 的站裁剪。  
- 列：`station_id`, `data_time`, 该库主序列列。  
- 点数上限：`DATA_QUERY_AGENT_HUD_MAX_POINTS_PER_STATION`（默认 200，按时间降采样或等间隔）。  
- 站数上限：`DATA_QUERY_AGENT_HUD_MAX_STATIONS`（默认 50）；超出则列表仍返回，HUD 只对前 N 站给序列，并 `warnings: hud_series_truncated`。

### 7.4 终局 `data_query_result` 契约
...
```json
{
  ...
  "hud_enabled": true,
  "columns": [
    {"key": "station_id", "title": "测位 ID"},
    {"key": "station_name", "title": "监测站名称"},
    {"key": "area", "title": "行政区"},
    {"key": "annual_settle_mm", "title": "年沉降量(mm)"},
    {"key": "total_settle_mm", "title": "历史累计沉降(mm)"},
    {"key": "data_time", "title": "数据时间"}
  ],
  "list": [
    {
      "station_id": "JZGZ",
      "station_name": "金盏公交",
      "area": "朝阳区",
      "annual_settle_mm": -84.2,
      "total_settle_mm": -418.5,
      "data_time": "2026-08-01T00:00:00",
      "hud_available": true
    }
  ],
  "hud_by_station": {
    "JZGZ": {
      ...
      "area": "朝阳区",
      "core_metrics": [
        {"id": "total_settle_mm", "name": "历史累计沉降", "value": -418.5, "unit": "mm"},
        {"id": "annual_settle_mm", "name": "年沉降量", "value": -84.2, "unit": "mm"}
      ],
      "series": {
        "metric": "total_settle",
        "unit": "mm",
        "points": [{"t": "2020-01-01", "v": -10.2}, {"t": "2021-01-01", "v": -90.0}]
      }
    }
  },
  "sql": {"q_list": "…", "q_hud_series": "…"},
  "warnings": []
}
```
...
- `hud_enabled=false` 时无 `hud_by_station`，行内不给 HUD 按钮。  
- `hud_enabled=true` 但某站被截断：该行 `hud_available=false`。  
- `columns` 随库变化（水位用埋深、GNSS 用位移），前端按 `columns` 动态表头，不要写死原型七列。  
- 负值表示下沉（沉降库）；与基座口径一致。  
- `sql` 仅 `expose_sql=true` 时出现。

HUD 页：前端用当前行 `station_id` 取 `hud_by_station[id]` 渲染指标卡 + 折线图，**无需再调算法**。

### 7.5 非站点维示例

问句「各区分层标最新平均沉降」→ `result_grain=district`，`hud_enabled=false`，`list` 为按区汇总行，无 HUD。
...
3. 监听 `data_query_result` → 渲染表；`hud_enabled` 控制操作列；点「加载 HUD」读 `hud_by_station`。  
...
| GET | `/data-query-agent/traces` 等 | P1，形态对齐 analysis_agent |
...
└── data_query_agent/
    ...
    ├── assemble.py          # grain / hud_enabled / 拼 hud_by_station
...
| `DATA_QUERY_AGENT_CHECKPOINT_BACKEND` | `redis` | HITL checkpointer；无 Redis 的本地/单测可设 `memory` |
| `DATA_QUERY_AGENT_SESSION_STORE_BACKEND` | `memory` | resume_token |
| `DATA_QUERY_AGENT_ACQUIRE_MAX_PARALLEL` | `2` | |
| `DATA_QUERY_AGENT_HUD_MAX_STATIONS` | `50` | |
| `DATA_QUERY_AGENT_HUD_MAX_POINTS_PER_STATION` | `200` | |
| `DATA_QUERY_AGENT_DEFAULT_MAX_ROWS` | `500` | |
| `DATA_QUERY_AGENT_TRACE_BACKEND` | `memory` | P1 `redis` |
...
| P0-D.4 | `result_grain` / `hud_enabled`：计划侧+结果侧校验 | §7.2 | 「各区平均」false；站点列表 true |
| P0-D.5 | `q_hud_series`：同 `forced_tables`；站数/点数上限；assemble 裁剪 | §7.3 | 超限 `hud_series_truncated`；行上 `hud_available` |
| P0-D.6 | 两路并行（max 2）；`include_hud=false` 只跑 q_list | §7.3、§4.1 | 并行结果按站对齐 |
| P0-D.7 | `assemble`：`columns` 随库变化；`list` + `hud_by_station`；负值下沉口径 | §7.4 | 列表与 HUD **同一 payload** |
...
4. 站点维 `hud_enabled=true` 且 HUD 折线在同一 `data_query_result`；区汇总无 HUD。  
...
| P1-5 | HITL session store 默认仍可为 memory；生产改为 redis（与已默认 redis 的 checkpoint 对齐） | §6.3、§10 | 多 worker resume 不丢 |
| P1-6 | HUD 上限可配并在 result.warnings 中可观测 | §7.3、§10 | 截断策略可运维调整 |
| P1-7 | 七库 `columns` / HUD 指标卡文案配置化（避免代码写死中文标题） | §7.4 | 气象双序列（温/雨）若做，在配置中声明 |
...
| P2-2 | `GET /data-query-agent/hud?library_id=&station_id=` 供 Java 默认表/点树列表 | §7.1、§14 | P0 解析路径仍是一组数据，本接口仅为浏览路径复用 |
| P2-3 | 与 NL2SQL golden 对齐的查询台回归集 | §12 原 P2 | 地降 PG 可自动跑 |
| P2-4 | 前端第三节全文（事件、HITL、动态列、HUD） | §16、§14 | 与实现零差 |
...
| **前端** | 解析按钮、空输入禁用、SSE 监听 hit/HITL/result、HUD 读 `hud_by_station`；**不做过滤器** | P0-E.1 事件名冻结后即可接；HITL 选项用中断事件 |
...
| **P2** | 产品扩展 | 双库策略（若需要）；Java HUD 补拉；文档与黄金集 | 单独立项评审后再做 |
...
| 「各区平均沉降」（已锁定 fcb） | 仍查 `t_data_wash_fcb`；hud_enabled=false |
...
| 过滤器 | 不做 |
| HUD | 解析路径用本模块 payload；Java 列表若无序列，P0 可隐藏 HUD 或 P2 再接补拉接口 |
...
3. **列表行数很大时 HUD 序列成本**：P0 用站数/点数上限；是否改为点击 HUD 再拉序列（违反「一组数据」）？建议 P0 遵守一组数据 + 上限；超限行 `hud_available=false`。  
...
- `地降所项目前端UI及对应接口调用说明.md` **第三节 数据查询**（现为空）。  
