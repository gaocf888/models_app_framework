...
> 5. 补齐生产口径：年沉降为窗内 **初值−末值**（与季报脚本 / P6 口径一致；无时间则近 365 天，不用「昨天」）；HITL session 与 checkpointer 双写（默认 redis checkpoint，session 未命中可回读）；`on_link_failure=refuse`；结构化日志含区/站/时间；地降 PG 集成测默认 skip（`NL2SQL_PG_SMOKE=1`）。查询台进程须 `NL2SQL_BUSINESS_DOMAIN=subsidence`。  
...
> 9. **2026-09-11**：周期沉降符号与三维 grain 对齐 [`NL2SQL基座改造补充-分层标数据汇总逻辑(三维口径与层位字典)实现方案.md`](./NL2SQL基座改造补充-分层标数据汇总逻辑(三维口径与层位字典)实现方案.md)（**初−末、正下沉**；旧稿「终−初/负下沉」废止）。  
...
| 原型列 | 物理库 | 本模块策略 |
|--------|--------|------------|
| 年滑速 (mm/yr) | **无此列** | **不造列**。问「年沉降/年速率」用近 12 个月（或解析出的年窗）`total_settle` **初值−末值**（与 [P6 口径专项](./NL2SQL基座改造补充-分层标数据汇总逻辑(三维口径与层位字典)实现方案.md) 一致；**Δ>0 下沉倾向**）；展示字段名用 `annual_settle_mm`（或库对应年度变化） |
...
并行 max 2；`include_hud=false` 只跑 `q_list`。两路同一 `forced_tables` 与区/站/时间。`q_list` 站点约束同前（每站最新、库度量列、年变化窗内**初值−末值**、LIMIT）。
