...
-- 占位符（与 img_diag_defect_ident_plan_reference_sql.sql 一致；分级：机组→受热面→检测位置→排数→管数）：
--   @unit_keyword / @device_keyword / @location_keyword / @row_no / @tube_no / @t_start / @t_end
-- 说明：未解析下级字段置 NULL/空串，guard 自动跳过该条件。
