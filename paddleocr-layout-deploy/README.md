...
- `Dockerfile.cpu`：`ARG BASE_IMAGE` 默认 **`docker.m.daocloud.io/library/python:3.11-bookworm`**（绕过常见失效的 `docker.io` → `hub-mirror.c.163.com` 配置）；compose 通过 **`PADDLE_LAYOUT_CPU_BASE_IMAGE`** 覆盖；海外可设为 **`python:3.11-bookworm`**。
...
# 若 DaoCloud 不可达，在 .env 中改为贵司可达的 python:3.11-bookworm 镜像源
