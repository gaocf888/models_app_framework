...
- **`plan_context_rag` → `merge_plan_tasks` → `acquire_data` → `data_quality_gate`**：与现有 nl2sql 路径同源（**`acquire_data`** 内 **`AnalysisGraphRunner._execute_data_plan`**：**默认同 dependency 层并行** **`NL2SQLService.query`**）；`merge_plan_tasks` 可对齐 `_merge_nl2sql_template_and_llm_tasks`，模板 scene 为 `analysis_plan_<img_diag_type>`。
...
### 5.2.1 看图诊断 · 流式 synthesis（已实现）

- **路径**：`POST /analysis/run-img-diag-stream`
- **请求体**：与 **`/analysis/run-img-diag`** 相同（**`AnalysisImgDiagRequest`**）。
- **响应**：`text/event-stream`，事件序列与 **`POST /analysis/run-with-nl2sql-stream`** 对齐（**`meta` → `summary_delta` → `summary_complete` → `structured_async_enqueued`**）；并行臂与同步接口一致，仅 synthesis 改为 **`_stream_summary_text`**，完整 JSON 异步写日志（**`analysis_img_diag_stream_full_json`**）并可注册 **`register_analysis_nl2sql_stream_structured_hook`** 投递；**trace** 在后台 **`_save_trace`**。
- **用途**：首屏逐步展示结论段落；移动端弱网下可读性更好。
- **scope HITL（已实现）**：缺机组/受热面或库表未匹配时先下发 **`img_diag_scope_input_required`**，再经 **`POST /analysis/run-img-diag-resume-stream`** 续跑。库未匹配达独立轮次阈值后可进入 **`hitl_mode=scope_candidate_pick`**（`ui_buttons` + `scope_patch`/`candidate_pick_*`）。契约细节见企业级文档《看图诊断(缺陷识别&泄爆分析)实现和使用说明》与 `app/api/analysis.py` docstring。
...
4. 在 `analysis.py` 注册 `POST /analysis/run-img-diag`、`POST /analysis/run-img-diag-stream`，补齐 OpenAPI 描述。
