...
- 接口：
  ...
  - `POST /chatbot/chat/stop`（显式中断指定 `stream_id`）。
...
- **图片预处理（入口前）**：`ChatbotService` 在进入图/legacy 链路前会对 `image_urls` 做预处理（下载、最长边缩放、超阈值有损压缩、落盘），并替换为本地静态 URL；静态访问前缀默认 **`/chatbot/media`**。  
...
- 流式响应：
  ...
  - 完成事件：`{"finished":true,"meta":{...}}`（`meta` 含 **`stream_id`**、**`suggested_questions`**、**`used_nl2sql`**、**`nl2sql_sql`**（若有）、**`intent_label`**、**`rag_namespace` / `rag_scope_reason` / `rag_scope_fallback`**（本厂 RAG 范围，若有）、**`processed_image_urls`**（预处理后图片 URL 列表）等）；
...
- 典型流程：
  ...
  3. **`kb_qa`**：`select_rag_engine` → **`rag_scope_resolve`（本厂指代 → 锁定电厂专属 namespace）** → **`kb_retrieve`**（指代消解 P0/P2/P3 + 带 namespace 召回）→ C-RAG 循环 → **`kb_build_messages`**（可选 **对话锚块 P1**）→ Runner 层流式生成；
...
- 统一入口 `app/llm/graphs/chatbot_intent.py`：`CHATBOT_INTENT_BACKEND=rules|bert`（默认 **rules**）。
- **rules**：`chatbot_intent_rules.classify_chatbot_intent_by_rules` — 台账/统计/列表/检修记录等 → `data_query`；原因/机理/标准/预防等 → `kb_qa`；带图固定 `kb_qa`。
- **bert**：`chatbot_intent_bert` — 硬规则闸与 rules 共用，其余由**已微调** BERT 三分类（`AutoModelForSequenceClassification`）；**不支持**魔塔/HF 通用预训练 BERT 直接上线；加载失败且 `CHATBOT_INTENT_BERT_FALLBACK_TO_RULES=true` 时回退 rules。不想训练请用 `rules`。
- 意图标签受 **`CHATBOT_INTENT_OUTPUT_LABELS`** 约束；未放量的标签会降级为 `kb_qa`。
- 低检索质量时走 C-RAG 重写重试，超过预算后转 `clarify`；  
- **本厂专属知识库（RAG 范围）**：`rag_scope_resolve` 根据厂别指代（如本厂/本公司/本电厂等）将主检索 **限定为** `CHATBOT_PLANT_KB_NAMESPACE`（默认 `Power_plant_knowledge`）；结果写入 state 的 `rag_namespace`，**C-RAG 重试**（`kb_rewrite_query → kb_retrieve`）**复用同一 namespace**，不再经过 `rag_scope_resolve`；  
...
- 图开关与意图：`CHATBOT_GRAPH_ENABLED`、`CHATBOT_INTENT_ENABLED`、**`CHATBOT_INTENT_BACKEND`（rules|bert）**、**`CHATBOT_INTENT_OUTPUT_LABELS`（默认含 `data_query`）**；BERT 见 **`CHATBOT_INTENT_BERT_*`** 与 `docs/智能客服意图识别BERT接入说明.md`；
...
- **本厂专属知识库 RAG 范围**：`CHATBOT_PLANT_KB_ENABLED`、`CHATBOT_PLANT_KB_NAMESPACE`、`CHATBOT_PLANT_KB_QUERY_BOOST_NAME`、`CHATBOT_PLANT_KB_FALLBACK_ON_EMPTY`（首轮 plant 库 0 命中是否回退全库；C-RAG 重试仍保持 plant namespace）；
...
- **上下文指代消解（可选）**：`CHATBOT_ANAPHORA_RETRIEVAL_FUSION_ENABLED`、`CHATBOT_ANAPHORA_CONFIG_PATH`、`CHATBOT_ANAPHORA_FUSION_MAX_CHARS`、`CHATBOT_ANAPHORA_ANCHOR_BLOCK_ENABLED`、`CHATBOT_ANCHOR_BLOCK_MAX_CHARS`、`CHATBOT_ANAPHORA_SLOTS_ENABLED`、`CHATBOT_ANAPHORA_SLOTS_MAX_BULLETS`、`CHATBOT_ANAPHORA_LLM_GATE_ENABLED`、`CHATBOT_ANAPHORA_LLM_TIMEOUT_SEC`、`CHATBOT_ANAPHORA_LLM_MODEL`、`CHATBOT_ANAPHORA_EXPOSE_META`（说明见同文档 **第 15 节** 与 `docs/智能客服上下文理指代实现优化方案-20260514.md`）。
- 图片预处理（入口前）：`CHATBOT_IMAGE_PREPROCESS_ENABLED`、`CHATBOT_IMAGE_MAX_EDGE`、`CHATBOT_IMAGE_COMPRESS_THRESHOLD_MB`、`CHATBOT_IMAGE_JPEG_QUALITY`、`CHATBOT_IMAGE_STORE_DIR`、`CHATBOT_IMAGE_PUBLIC_PATH`。
...
| 图片预处理 | `app/services/chatbot_image_preprocessor.py` | 下载、缩放、压缩、落盘并返回本地可访问 URL |
...
| 结构化索引/引用解析 | `app/services/chatbot_outline.py` | 回答后结构化索引提取、Redis/ES 持久化、上文第 N 点解析 |
| 指代消解（P0～P3） | `app/llm/graphs/chatbot_retrieval_query.py`、`chatbot_anaphora_*.py`、`chatbot_dialogue_anchor.py` | 检索融合、规则、槽位、Coref LLM；清单 `configs/chatbot_anaphora.yaml` |
...
**其余**：与「上文第 N 点」结构化索引互补；**P3** 默认关、以图路径为主。**配置与流程图**：`enterprise-level_transformation_docs/企业级智能客服 LangGraph 框架实现方案.md` **§15.0～15.1**；专项 `docs/智能客服上下文理指代实现优化方案-20260514.md` **§3.3**；清单 `configs/chatbot_anaphora.yaml`。
