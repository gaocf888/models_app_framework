...
- 流式响应：
  ...
  - 完成事件：`{"finished":true,"meta":{...}}`（`meta` 含 **`stream_id`**、**`suggested_questions`**、**`used_nl2sql`**、**`nl2sql_sql`**（若有）、**`intent_label`**、**`processed_image_urls`**（预处理后图片 URL 列表）等）；
...
- 典型流程：
  ...
  3. **`kb_qa`**：在 **`kb_retrieve`** 中可选执行**指代消解**（规则 P0、槽位 P2、灰区 Coref P3），再 RAG 引擎选择与 C-RAG 循环 → **`kb_build_messages`**（可选 **对话锚块 P1**）→ Runner 层流式生成；
