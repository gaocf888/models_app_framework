...
> **实现状态**：**payload / nl2sql 双入口** 由 `AnalysisGraphRunner` 内两套 LangGraph `StateGraph` 编排；**看图诊断（`img_diag`）** 由子类 **`AnalysisImgDiagGraphRunner`** 以 **`asyncio.gather` 并行三臂 + 汇合合成** 实现（**未编译为第三套 StateGraph**，trace 中 `orchestrator=asyncio_gather_parallel`）。可选检查点与 NL2SQL 多阶段 LLM 计划已落地；下文流程图与节点名以 payload/nl2sql 为主，**看图诊断专项说明与逻辑图**见 **`enterprise-level_transformation_docs/企业级综合分析-看图诊断实现和使用说明.md`**。
...
1. 提供运行入口：
   ...
   - `POST /analysis/run-img-diag`（看图诊断同步：`AnalysisV2Result`）
   - `POST /analysis/run-img-diag-stream`（看图诊断流式 synthesis：`text/event-stream`，事件对齐 `run-with-nl2sql-stream`）
   - 看图诊断均需：图片 URL + **机组 ID** + 位置 + 提问；可先 `POST /analysis/img-diag/upload`
2. **payload / nl2sql** 共用 **`AnalysisGraphRunner`**，**编译为两套 `StateGraph`**（节点边不同）；**看图诊断** 在同一服务内由 **`AnalysisImgDiagGraphRunner`** 扩展，并行执行「视觉 LLM ‖ NL2SQL 子链路（至 `data_quality_gate`）‖ 业务 RAG」后再合成，语义上为 **第三 API 入口、独立编排策略**。
...
### 2.4 看图诊断（`img_diag`，补充）

1. **请求模型**：`AnalysisImgDiagRequest`（`app/models/analysis.py`），必填 **`unit_id`、`leak_location_text`、`query`、`image_urls`**；`analysis_type` 在 NL2SQL 桥接请求中固定为 **`img_diag`**。  
2. **数据计划**：`configs/prompts.yaml` 场景 **`analysis_plan_img_diag`**；占位符 **`{unit_id}`、`{location}`、`{location_struct}`** 在计划合并后、取数前替换。  
3. **RAG**：规划前仍在 NL2SQL 臂内走 **`scene=nl2sql`**；业务解释在 **并行臂** 内走 **`scene=analysis`**（**不**再走 nl2sql 主图节点 **`rag_enrichment`**，避免与并行业务 RAG 重复）。  
4. **产出**：`AnalysisV2Result` 中 **`evidence.data_coverage.mode`** 为 **`img_diag`**，**`evidence.vision_findings`** 承载视觉 JSON；**`parallel_lane_trace`** 记录各臂耗时与状态。
...
4. `acquire_data`：`_execute_data_plan` 按 `plan_tasks` 调度 NL2SQL（**默认同 dependency 层并行** `NL2SQLService.query`，可 **`ANALYSIS_NL2SQL_ACQUIRE_PARALLEL_ENABLED`** 关闭）；每次 `query` 传 **`time_intent_text=req.query`**，由链内 **`resolve_question_intent`** 解析时间/范围并改写 SQL；trace 可选含 **`question_intent`**（`NL2SQL_TRACE_INCLUDE_QUESTION_INTENT`）。依赖、重试、跳过见 §7.3 / §9。  
5. `data_quality_gate` → `rag_enrichment`（`scene=analysis`）→ `synthesis`：`_build_summary_messages` 拆 **system**（`analysis_synthesis_*` 模板）与 **user**（`gathered_data`、`context_snippets`、可选 `planning_context`）；`llm.chat` / `stream_chat`；`gathered_data` 截断由 **`ANALYSIS_SYNTHESIS_GATHERED_JSON_MAX_CHARS`**（默认 16000）控制；输出 **`ANALYSIS_SYNTHESIS_MAX_TOKENS`**（默认 8192）。  
...
### 5.3 `POST /analysis/run-img-diag` / `POST /analysis/run-img-diag-stream`（看图诊断）

**同步**与**流式**共用下列请求体（**`AnalysisImgDiagRequest`**）；流式响应为 SSE，完整 JSON 异步落日志 / trace（见企业文档）。

另需 **`POST /analysis/img-diag/upload`**（`multipart/form-data`，字段 `file`）上传现场图，响应中的预签名 URL 填入下例 **`image_urls`**。
...
```json
{
  ...
  "image_urls": ["https://minio.example/.../analysis_img_diag/xxx.jpg"],
...
详见 **`enterprise-level_transformation_docs/企业级综合分析-看图诊断实现和使用说明.md`**。
...
- `nl2sql`：**`_execute_data_plan`**：按 **`dependency_ids` 分层**；每层 **Runnable** 任务默认 **`asyncio.gather`** 并行调用 **`NL2SQLService.query`**（**`ANALYSIS_NL2SQL_ACQUIRE_MAX_PARALLEL`** 限并发；**`ANALYSIS_NL2SQL_ACQUIRE_PARALLEL_ENABLED=false`** 时整段串行）。单条 **`query`** 内仍为「生成 SQL → 执行」串行。
...
- **看图诊断（`img_diag`）**：**不经过**本节点；业务向 RAG 在 **`run_with_img_diag`** 中与视觉、NL2SQL **并行**调用 `_retrieve_business_rag`，合成阶段再并入 `context_snippets` 等。  
- 证据链：`rag_sources` 含 `namespace` / `doc_id` / `score`（受索引元数据完整度影响）。

### 7.6 `synthesis_node`（v1 / v2 双策略）

**策略选择**：`_execute_synthesis`；配置 `ANALYSIS_SYNTHESIS_STRATEGY` + `ANALYSIS_SYNTHESIS_STRATEGY_<专项>`；`img_diag` 固定 v1。trace 记录 `synthesis_strategy` / `synthesis_strategy_effective`。

| 策略 | 实现 | 要点 |
|------|------|------|
| **v1（默认）** | `_build_summary_messages` + `chat`/`stream_chat` | system=`analysis_synthesis_*`；user=`gathered_data`+RAG+可选规划意图 |
| **v2** | `app/llm/graphs/analysis_synthesis_v2.py` | **14 槽**（8×LLM + 表/图/静态）；LLM 槽 system=`analysis_synthesis_overheat_narrative`，user=**q 切片**+`narrative_instruction`；**不**注入 `analysis_synthesis_overheat_guidance` v2 整篇；按序流式 `summary_delta`；P0 仅 `overheat_guidance`；无注册回退 v1 |

**v2 提示词与数据（超温）**

| 组件 | 场景 / 位置 | 运行时 |
|------|-------------|--------|
| 九段报告蓝本 | `analysis_synthesis_overheat_guidance` v2 | 设计文档；**v1 整篇** synthesis 的 system |
| 叙述 system | `analysis_synthesis_overheat_narrative` | **所有 LLM 槽** |
| 分章任务 | `_overheat_v2_slots().narrative_instruction` | **各 LLM 槽 user** |
| 取数 | `analysis_plan_overheat_guidance` v2（q1～q6） | `acquire_data` |

参考：`enterprise-level_transformation_docs/系统整体技术实现-简版.md` §1.1.4～1.1.6。

**v1 参数**：`ANALYSIS_SYNTHESIS_GATHERED_JSON_MAX_CHARS`（默认 16000）、`ANALYSIS_SYNTHESIS_MAX_TOKENS`（默认 8192）、`ANALYSIS_SYNTHESIS_TIMEOUT_SECONDS`（90s）。

**v2 参数**：`ANALYSIS_SYNTHESIS_V2_MAX_PARALLEL_LLM`、`ANALYSIS_SYNTHESIS_V2_SEGMENT_MAX_TOKENS` 等（见 `.env.example`）。

**plan 版本联动**：`ANALYSIS_PLAN_TEMPLATE_VERSION`（及专项覆盖）→ `acquire_data` 使用 `analysis_plan_<type>` 对应 version；与 QA 写入五元组一致。详案：`docs/综合分析优化版本实现方案(v2版本).md`。
...
### 10.1 阶段模板读取优先级（代码真实行为）

`AnalysisGraphRunner._resolve_stage_template(stage, analysis_type, ...)` 的查找顺序为：

1. `<stage>_<analysis_type>`
2. `<stage>`
3. `analysis`（最终兜底）
...
以 `analysis_type=overheat_guidance` 为例，四个阶段分别按以下顺序读取：
...
### 10.2 数据计划模板读取（与阶段模板不同）

NL2SQL 数据计划由 `_build_data_plan_from_template` 按 `scene=analysis_plan_<analysis_type>` 读取（例如 `analysis_plan_overheat_guidance`）。
若该模板缺失或 JSON 非法，则回退到 runner 内置默认任务。

### 10.3 prompts.yaml 键名建议（与代码一致）

建议在 `configs/prompts.yaml` 扩展以下键：

1. `analysis_intent_<analysis_type>`
2. `analysis_data_plan_<analysis_type>`
3. `analysis_synthesis_<analysis_type>`
4. `analysis_report_<analysis_type>`
5. `analysis_plan_<analysis_type>`（数据计划 JSON）
...
1. `app/api/analysis.py`
   - 企业版运行入口：`/analysis/run-with-payload`、`/analysis/run-with-nl2sql`、`/analysis/run-with-nl2sql-stream`、**`/analysis/run-img-diag`**、**`/analysis/run-img-diag-stream`** 与 **`/analysis/img-diag/upload`**；及 trace 运维接口。
...
4. `app/services/analysis_service.py`
   - `run_analysis_payload` / `run_analysis_nl2sql` / `run_analysis_nl2sql_stream` / **`run_analysis_img_diag`** / **`run_analysis_img_diag_stream`**，组装 **`AnalysisImgDiagGraphRunner`**（兼容 payload/nl2sql/img_diag）、trace 持久化与检索。
5. `app/llm/graphs/`
   - `analysis_graph_state.py`、`analysis_graph_runner.py`，以及 **`analysis_img_diag_runner.py`**（看图诊断并行编排；nl2sql 节点逻辑仍复用父类）。
6. RAG 与 NL2SQL
   - RAG 复用现有服务；
