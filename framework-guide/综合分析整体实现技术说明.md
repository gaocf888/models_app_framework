...
4. `acquire_data`：`_execute_data_plan` 按 `plan_tasks` 调度 NL2SQL（**默认同 dependency 层并行** `NL2SQLService.query`，可 **`ANALYSIS_NL2SQL_ACQUIRE_PARALLEL_ENABLED`** 关闭）；依赖、重试、跳过见 §7.3 / §9。  
