...
- **提示词形态**：`_build_summary_messages` → OpenAI 兼容 **`messages`**（非单条 user 拼接）：
  - **`role=system`**：`_resolve_stage_template(stage="analysis_synthesis")`（如 `analysis_synthesis_overheat_guidance`）。
  - **`role=user`**：`_build_summary_user_content` — 分析类型、数据模式、用户问题、可选 **`planning_context`**（`intent_llm_result` JSON，≤2000 字符）、**`数据摘要(JSON截断)`**（`json.dumps(gathered_data)[:N]`，`N=ANALYSIS_SYNTHESIS_GATHERED_JSON_MAX_CHARS`，默认 **16000**）、**`RAG参考片段`**（最多 8 条）。
- **调用**：`_generate_summary` → `VLLMHttpClient.chat`；流式 → `stream_chat`（`iter_nl2sql_stream_events` / 看图诊断流式）。
- **输出**：`summary`、`suggestions`；LLM **`max_tokens`** = **`ANALYSIS_SYNTHESIS_MAX_TOKENS`**（默认 **8192**）；超时 **`ANALYSIS_SYNTHESIS_TIMEOUT_SECONDS`**（默认 90s）。
- **约束**：报告结构以 system 模板为准；须引用事实数据，不得编造具体数值（v2 程序出表方案见 `docs/综合分析优化版本实现方案(v2版本).md`）。
