...
1. 提供运行入口：
   ...
   - `POST /analysis/run-img-diag`（看图诊断：图片 URL + **机组 ID** + 位置 + 提问；可先 `POST /analysis/img-diag/upload`）
...
### 5.3 `POST /analysis/run-img-diag`（看图诊断）
...
1. `app/api/analysis.py`
   - 企业版运行入口：`/analysis/run-with-payload`、`/analysis/run-with-nl2sql`、**`/analysis/run-img-diag`** 与 **`/analysis/img-diag/upload`**；及 trace 运维接口。
...
4. `app/services/analysis_service.py`
   - `run_analysis_payload` / `run_analysis_nl2sql` / **`run_analysis_img_diag`**，组装 **`AnalysisImgDiagGraphRunner`**（兼容 payload/nl2sql/img_diag）、trace 持久化与检索。
5. `app/llm/graphs/`
