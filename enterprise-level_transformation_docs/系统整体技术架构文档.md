...
> **范围**：覆盖 `app/main.py` 挂载的 **健康检查、通用推理、智能客服、综合分析、NL2SQL、RAG 管理** 及公共基础设施。  
...
| **功能** | 综合分析 V2 | `POST /analysis/run-with-*` | **LangGraph**（`AnalysisGraphRunner`）大规模节点编排 | Payload / NL2SQL 双入口；trace 存储 |
...
**智能客服（`/chatbot`）**  
编排：**LangGraph** 状态图（意图分类 → RAG 引擎选择 → 可选 C-RAG → 消息构建 → **`VLLMHttpClient.stream_chat`**）；可 **Legacy** 回退。  
框架：**LangGraph** 为主；装配 **会话基座、RAG、NL2SQL、Prompt 注册表**；**SSE** 与 REST 会话同进程。

**综合分析 V2（`/analysis`）**  
编排：**payload / nl2sql 双入口** → **大规模 LangGraph**（数据覆盖、NL2SQL 多轮、图表、报告、strict 等节点）；**trace** 旁路持久化。  
框架：**LangGraph** + 多基座（RAG、NL2SQL、LLM、会话/trace）；归档语义见专项文档。
...
| `/analysis` | 综合分析 V2（payload / nl2sql）、trace 运维查询 | 同上 |
...
- **双入口**：`POST /analysis/run-with-payload`（调用方直接给分析载荷）、`POST /analysis/run-with-nl2sql`（系统内多次 NL2SQL 取数后再生成报告）。  
- **编排**：`AnalysisGraphRunner`（`app/llm/graphs/analysis_graph_runner.py`），企业版结构化输出与质量/strict 策略由 `AnalysisConfig` 约束。  
- **可观测性**：trace 列表、单条回放、统计与趋势等运维接口（存储后端 `trace_backend`：Redis / 内存，归档可选 ES 相关配置）。
