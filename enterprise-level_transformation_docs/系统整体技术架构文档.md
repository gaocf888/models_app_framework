...
**智能客服（`/chatbot`）**  
编排：请求进入 `ChatbotService` 后先做图片预处理（缩放/压缩/本地落盘）再进入 **LangGraph**（意图分类 → **可选意图 HITL** → RAG / NL2SQL 分支 → **本厂 namespace 解析（`rag_scope_resolve`）** → 可选 C-RAG → 消息构建 → **`VLLMHttpClient.stream_chat`**；查库 SQL 生成失败可 HITL）；可 **Legacy** 回退。  
框架：**LangGraph** 为主；装配 **会话基座、RAG、NL2SQL、Prompt 注册表、HITL 续跑**；通过 `StaticFiles` 暴露本地图片路径（默认 `/chatbot/media`）；SSE `finished.meta` 额外返回 `processed_image_urls`。
...
- **典型节点能力**：历史加载、意图分流（如 `kb_qa` / `clarify` / `data_query`）、**可选 HITL**（意图确认 / 二次消歧 / NL2SQL 生成失败，`/chat/resume-stream`）、RAG 检索与 C-RAG 重写、**data_query 路径内嵌 NL2SQL**、结束元数据（如推荐追问）。可选 checkpoint（memory / redis）。  
