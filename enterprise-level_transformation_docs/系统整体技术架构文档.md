...
| **检修报告结构化提取 V0** | **`enterprise-level_transformation_docs/企业级检修报告结构化提取V0版本实现和使用说明.md`**（含 **`paddleocr-layout-deploy`**） |
...
| **基座** | 嵌入向量 | RAG / NL2SQL 子检索 | **双后端**：默认 **MIS-TEI HTTP**（`EMBEDDING_BACKEND=mis_tei`）；或 **sentence-transformers**（`local`，离线路径优先） | `MIS_TEI_*` 或本地 `EMBEDDING_MODEL_PATH` / 模型名 |
| **基座** | 向量与全文检索 | 在线检索、摄入写索引 | **Elasticsearch 兼容 API**（含 EasySearch）；可选 **FAISS** | `RAG_ES_*`；索引别名与版本迁移 |
| **基座** | 检索库多路召回与融合 | 在线 RAG 主链路 | **`RAGService`**：多路召回 → **RRF** → 重排（默认 MIS-TEI `/rerank`，或 `local` CrossEncoder） | `RAGService`、`RAGConfig.hybrid`；`RAG_RERANKER_BACKEND` |
...
| **功能** | 综合分析 V2 | `POST /analysis/run-with-payload`、`/run-with-nl2sql`、`/run-with-nl2sql-stream`、**`/run-img-diag`**、**`/run-img-diag-stream`**（及 **`/img-diag/upload`**） | **LangGraph**（`AnalysisGraphRunner`，payload/nl2sql **两套 StateGraph**）+ **`AnalysisImgDiagGraphRunner`**（看图诊断 **`asyncio.gather` 并行三臂 + 合成 / 流式 synthesis） | Payload / NL2SQL **+ 看图诊断**；trace 存储；专题见 **`企业级综合分析-看图诊断实现和使用说明.md`** |
...
| **功能** | **检修报告结构化提取 V0** | **`POST /inspection-extract-v0/upload`**、**`…/run`**、**`…/run/async`**、**`…/jobs/*`** | **LangGraph** 单阶段 LLM + **HTTP 版面侧车**（`paddleocr-layout-deploy`） | MinIO；**`paddleocr-layout-api`**（`INSPECT_EXTRACT_V0_LAYOUT_OCR_ENDPOINT`）；可选关闭 docx 侧车 |
...
**RAG 检索与知识基座**  
检索：多路并行召回（语义向量 + 关键词/全文 + 可选元数据）→ RRF 融合 → 重排（默认 **MIS-TEI**，或 `local` CrossEncoder）→ top_k 截断（重排失败则保留 RRF 序）。
框架：自研分层（`RAGService` 负责向量/关键词混合与重排，`HybridRAGService` 叠图路由，`AgenticRAGService` 子查询与预算）；嵌入默认 **MIS-TEI HTTP**（亦可 `local` sentence-transformers），存储经 **`VectorStoreProvider`** 对接 **ES/EasySearch 兼容 API** 或 **FAISS**；可选 **GraphRAG**（Neo4j + **LangChain Graph**）；摄入为 **异步 Job**，参数由 **`RAGConfig`** 聚合。
...
**智能客服（`/chatbot`）**  
编排：请求进入 `ChatbotService` 后先做图片预处理（缩放/压缩/本地落盘）再进入 **LangGraph**（意图分类 → RAG 引擎选择 → **本厂 namespace 解析（`rag_scope_resolve`）** → 可选 C-RAG → 消息构建 → **`VLLMHttpClient.stream_chat`**）；可 **Legacy** 回退。  
...
框架：`InspectionExtractService` + `InspectionExtractLlmOrchestrator`；**V2 管线**（`INSPECT_EXTRACT_PIPELINE_VERSION=v2`）用 `app/inspection_v2` 序列化表格（含 `[颜色标注:高亮=…]`），分块 parse 后 **`color_guard`** 按壁厚单元格颜色校正检测类型（`INSPECT_EXTRACT_V2_COLOR_GUARD`）；规则层负责缺陷归一、阈值绑定、段落近邻匹配与可追溯 warnings。详见现网企业级说明 **§4.4**。

**检修报告结构化提取 V0（`/inspection-extract-v0`）**  
编排：**LangGraph**（预处理 → **版面 OCR HTTP 调用** → IRT 构建 → **单阶段 LLM** → 校验）；异步任务与现网目录/Redis 策略对齐但 **队列前缀独立**。  
框架：`InspectionExtractV0Service` + **`paddleocr-layout-deploy`** 侧车；企业级说明见 **`enterprise-level_transformation_docs/企业级检修报告结构化提取V0版本实现和使用说明.md`**。
...
- **检索与向量**：嵌入默认 **MIS-TEI HTTP**（`EMBEDDING_BACKEND=mis_tei`），亦可 **`local` sentence-transformers**；向量库抽象默认 **Elasticsearch 7.x API 兼容**（含 EasySearch），保留 **FAISS** 路径（配置 `RAG_VECTOR_STORE_TYPE` / `RAG_FAISS_INDEX_DIR`）；混合检索、RRF、重排（MIS-TEI 或 CrossEncoder）见 `HybridRetrievalConfig`。
...
| `/analysis` | 综合分析 V2（payload / nl2sql / nl2sql-stream / **看图诊断 `run-img-diag` / `run-img-diag-stream`**）、**`img-diag/upload`**、trace 运维查询 | 同上 |
| `/inspection-extract` | 检修报告结构化提取（upload/run） | 同上 |
| **`/inspection-extract-v0`** | **检修结构化 V0**（upload/run/async/jobs；依赖可选 **`paddleocr-layout-deploy`**） | 同上 |
...
- **运行入口**：`POST /analysis/run-with-payload`（调用方直接给分析载荷）、`POST /analysis/run-with-nl2sql`（及流式 **`run-with-nl2sql-stream`**）、**`POST /analysis/run-img-diag`**（及流式 **`run-img-diag-stream`**：机组位置 + 图片 URL + 提问；可先 **`POST /analysis/img-diag/upload`**）。  
...
| 检修报告结构化提取 | `enterprise-level_transformation_docs/企业级检修报告结构化提取实现和使用说明.md` |
| **检修报告结构化提取 V0** | **`enterprise-level_transformation_docs/企业级检修报告结构化提取V0版本实现和使用说明.md`**、`paddleocr-layout-deploy/README.md` |
