...
| scope HITL | `app/llm/graphs/img_diag_scope_graph.py` | LangGraph 子图：`scope_preflight_llm` → `scope_human_confirm` → `scope_db_validate` |
| scope 辅助 | `img_diag_scope_intent.py`、`img_diag_scope_validate.py`、`img_diag_scope_display.py` | 解析、库表校验、中文展示 |
...
### 5.2 scope 人机协同（两层 HITL）

**触发条件（`should_trigger_scope_hitl`）**：

| 层级 | 条件 | 行为 |
|------|------|------|
| **L1 解析** | 缺 `boiler` 或 `device_name` | `scope_human_confirm` → LangGraph `interrupt` |
| **L2 库表** | `ANALYSIS_IMG_DIAG_SCOPE_VALIDATE_SQL` COUNT=0 或 SQL 错误 | 再次 interrupt，提示台账未匹配 |

**子图节点**：

1. **`scope_preflight_llm`**：`parse_img_diag_scope_draft`（规则 + LLM，场景 `nl2sql_scope_parse`）
2. **`scope_human_confirm`**：`interrupt` 抛出 HITL payload；用户 resume 后合并 `user_supplement` / `scope_patch`
3. **`scope_db_validate`**：账册 existence 校验；通过后产出 `confirmed_scope_intent` + `scope_intent_text`

**路由**：

- preflight 缺字段 → human_confirm
...
stateDiagram-v2
  ...
  scope_human_confirm --> scope_preflight_llm: confirm_scope / edit_scope + supplement
  scope_human_confirm --> [*]: abort
  scope_db_validate --> [*]: confirmed_scope_intent 就绪
  scope_db_validate --> scope_human_confirm: 库表无匹配 / SQL 错误
```
...
sequenceDiagram
  ...
  alt scope 需确认
    ...
    Note over FE: 展示 HITL，用户补充 query
...
| `payload.scope_patch` | 结构化修正（`boiler`/`device_name` 或中文键 `机组`/`受热面`） |
| `payload.reason` | `action=abort` 时可选 |

**响应 SSE**：确认通过后顺序与正常流相同（`meta` → `summary_delta` → … → `finished`）；若仍不通过，再次 `img_diag_scope_input_required`。

### 7.5 前端集成要点（状态机）

1. **首次**始终调 `/run-img-diag-stream`
2. 在同一条 SSE 连接内监听 `event === "img_diag_scope_input_required"`
3. 收到后存 `resume_token`，展示 `prompt` + `scope_draft_display`，用户补充后调 `/run-img-diag-resume-stream`
4. 未收到 HITL 事件则直接 `meta` → 报告流，无需 resume
5. resume 的 SSE 中可能**再次出现** HITL，重复上述步骤
...
| event | 何时出现 | 主要字段 |
|-------|----------|----------|
| `img_diag_scope_input_required` | scope 待确认（**可能在 meta 之前**）；确认前流结束 | `resume_token`、`prompt`、`scope_draft`、`scope_draft_display`、`missing_fields`、`validation_error`、`interrupt_reason`、`suggested_actions` |
...
| `ANALYSIS_IMG_DIAG_SCOPE_VALIDATE_SQL` | （内置 COUNT SQL） | 库表 existence 校验；须 `:boiler`、`:device_name` 占位符 |
...
| 场景键 | 用途 |
|--------|------|
| `nl2sql_scope_parse` | scope HITL preflight + NL2SQL 范围解析（`scene=nl2sql_scope_parse`） |
...
# 3. 若收到 img_diag_scope_input_required，续跑
