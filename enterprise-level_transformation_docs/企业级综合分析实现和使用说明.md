...
2. **编排内核**：**payload / nl2sql** 在 `AnalysisGraphRunner` 内 **`langgraph.StateGraph`** 编译执行；`langgraph` 不可用时 **顺序回退** 同一套异步节点，并在 `execution_summary.graph_orchestrator` 中区分。**看图诊断** 由 **`AnalysisImgDiagGraphRunner.run_with_img_diag`** 使用 **`asyncio.gather`** 三路并行后再合成（**未编译为第三套 StateGraph**）。
...
| 看图诊断编排 | `app/llm/graphs/analysis_img_diag_runner.py` | **`run_with_img_diag`**：并行视觉 / NL2SQL（至 `data_quality_gate`）/ 业务 RAG → **`_generate_summary`**；占位符替换 **`_substitute_img_diag_placeholders`**。 |
