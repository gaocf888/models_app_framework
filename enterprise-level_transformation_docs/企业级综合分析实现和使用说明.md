...
**主分析LLM(synthesis)**：经 `_build_summary_messages` 以 **system**（`analysis_synthesis_<analysis_type>`，如 `analysis_synthesis_overheat_guidance`）+ **user**（`gathered_data` JSON、`rag_enrichment` 片段等）调用 `llm.chat` / `stream_chat`；`gathered_data` 整包截断见 **`ANALYSIS_SYNTHESIS_GATHERED_JSON_MAX_CHARS`**（默认 16000）；输出 **`ANALYSIS_SYNTHESIS_MAX_TOKENS`**（默认 8192）；支持流式输出
...
| `ANALYSIS_SYNTHESIS_TIMEOUT_SECONDS` | `90` | synthesis **`_generate_summary`** / **`_stream_summary_text`** 超时（秒） |
| `ANALYSIS_SYNTHESIS_GATHERED_JSON_MAX_CHARS` | `16000` | synthesis **user** 消息中 `gathered_data` 经 `json.dumps` 后的最大字符数（≥1000） |
| `ANALYSIS_SYNTHESIS_MAX_TOKENS` | `8192` | synthesis LLM 输出 `max_tokens`（六章+附录多表建议 ≥8192，避免中途截断） |
