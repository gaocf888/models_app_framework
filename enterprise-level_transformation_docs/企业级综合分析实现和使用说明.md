...
已对齐项：
1. 运行入口：`/analysis/run-with-payload`、`/analysis/run-with-nl2sql`、**`/analysis/run-img-diag`**（及 **`/analysis/img-diag/upload`**）已落地。
2. **编排内核**：**payload / nl2sql** 在 `AnalysisGraphRunner` 内 **`langgraph.StateGraph`** 编译执行；`langgraph` 不可用时 **顺序回退** 同一套异步节点，并在 `execution_summary.graph_orchestrator` 中区分。**看图诊断** 由 **`AnalysisImgDiagGraphRunner.run_with_img_diag`** 使用 **`asyncio.gather`** 三路并行(RAG臂、视觉臂、NL2SQL臂膀)后再合成（**未编译为第三套 StateGraph**）。
...
| 分层 | 主要文件 | 关键职责 |
|---|---|---|
| API | `app/api/analysis.py` | **三运行入口**（payload / nl2sql / **img_diag**）+ **`img-diag/upload`** + trace 查询/列表/统计/趋势/TopN。 |
| 服务层 | `app/services/analysis_service.py` | **`run_analysis_*`**、默认值注入、调用 **`AnalysisImgDiagGraphRunner`**、trace 持久化与检索、统计聚合与监控打点。 |
| 编排内核 | `app/llm/graphs/analysis_graph_runner.py`（**`AnalysisGraphRunner`**）；`app/llm/graphs/analysis_img_diag_runner.py`（**`AnalysisImgDiagGraphRunner`**，继承前者） | **通用基座**：payload / nl2sql 两套编译 **`StateGraph`**、检查点、顺序回退与节点实现；含 NL2SQL 规划前 RAG、可选意图/计划 LLM、计划合并、**`acquire_data`（`_execute_data_plan`：默认同 dependency 层并行 `NL2SQLService.query`）**、质量门、业务 RAG、`synthesis`、`finalize`。**看图诊断**：在通用基座上由子类提供 **`run_with_img_diag`** — **`asyncio.gather`** 并行「视觉 ‖ NL2SQL 臂（**直接复用父类** `_lg_nl2sql_*` 直至 **`data_quality_gate`**）‖ 业务 RAG」，再 **`_generate_summary`** 与结果组装；**`_substitute_img_diag_placeholders`**。 |
...
**看图诊断（`img_diag`）**：上图 **`run-img-diag`** 进入 **`run_analysis_img_diag`** → **`AnalysisImgDiagGraphRunner.run_with_img_diag`**；顶层 **fan-out / join**、RAG 双通道边界与 Trace 字段说明见 **`enterprise-level_transformation_docs/企业级综合分析-看图诊断实现和使用说明.md`**（含详细 Mermaid）。  
说明：图中 **`BR` / `LG` / `SEQ`** 分支刻画 **`run-with-payload` / `run-with-nl2sql`** 是否走编译 LangGraph；**`run-img-diag` 外层并行编排不经该菱形分支**，NL2SQL 臂内部仍复用父类节点函数。
