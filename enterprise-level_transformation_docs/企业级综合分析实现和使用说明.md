...
**acquire_data(NL2SQL 生成sql+查数)**: NL2SQL完整实现链路(调用封装好的NL2SQL链路)，通过提示词模板analysis_plan_[analysis_type]配置调用LLM生成sql的主提示词(同时还包括用户传参的问题等)；其中各个q之间(q数量根据实际场景业务配置，默认超温分析配置了q1~q4)是根据dependency_ids并行的(提高效率)
**主分析 synthesis**：默认 **v1**（`_build_summary_messages` → 单次 `chat`/`stream_chat`）；可配置 **v2** 多槽位编排（`analysis_synthesis_v2.py`，P0 超温）。配置见 **`ANALYSIS_SYNTHESIS_STRATEGY`** 及专项覆盖；plan/synthesis 模板 version 见 **`ANALYSIS_*_TEMPLATE_VERSION`**。v1 参数：`ANALYSIS_SYNTHESIS_GATHERED_JSON_MAX_CHARS`（16000）、`ANALYSIS_SYNTHESIS_MAX_TOKENS`（8192）。详案：`docs/综合分析优化版本实现方案(v2版本).md`。
