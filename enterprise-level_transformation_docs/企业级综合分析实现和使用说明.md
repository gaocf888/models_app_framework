...
> **看图诊断（`img_diag`，第三 HTTP 入口）**专项：**`enterprise-level_transformation_docs/企业级综合分析-看图诊断实现和使用说明.md`**。  
> 参考写法：`enterprise-level_transformation_docs/企业级NL2SQL基座实现方案.md`。

> 当前企业级综合分析的LangGraph主链路是：格式化入参 -> 规划前RAG检索 -> 意图识别&数据计划 -> acquire_data(NL2SQL 生成sql+查数) -> 质量门 -> 主分析LLM前RAG -> 主分析LLM
**格式化入参**： _apply_defaults_nl2sql
**规划前RAG检索**：plan_context_rag
**意图识别&数据计划**：intent_llm / plan_llm (通过提示词模板中 analysis_intent_[analysis_type]和analysis_data_plan_[analysis_type]来执行LLM，根据[analysis_type]配置不同的分析场景的独赢提示词模板，[analysis_type]是api请求传递的(比如超温分析：overheat_guidance))
**acquire_data(NL2SQL 生成sql+查数)**: NL2SQL完整实现链路(调用封装好的NL2SQL链路)，通过提示词模板analysis_plan_[analysis_type]配置调用LLM生成sql的主提示词(同时还包括用户传参的问题等)；超温 **plan v2 为 q1～q6**（与默认 `max_nl2sql_calls=6` 对齐），各 q 默认无 dependency 可并行
**主分析 synthesis**：默认 **v1**（`_build_summary_messages` → 单次 `chat`/`stream_chat`，system=`analysis_synthesis_*`）；可配置 **v2** 多槽位编排（`analysis_synthesis_v2.py`，P0 超温 **14 槽**）。**v2 LLM 槽**：system=`analysis_synthesis_overheat_narrative`，user=绑定 **q 切片 JSON** + 槽位 **`narrative_instruction`**（**不**运行时注入 `analysis_synthesis_overheat_guidance` v2 整篇）。配置见 **`ANALYSIS_SYNTHESIS_STRATEGY`** 及专项覆盖；plan 模板 version 见 **`ANALYSIS_PLAN_TEMPLATE_VERSION_*`**。详案：`docs/综合分析优化版本实现方案(v2版本).md`；参考 SQL：`docs/综合分析-超温分析 报告模板及查询数据逻辑/overheat_plan_v2_reference_sql.sql`。

> 当前通用综合分析，支持的专项分析类型：超温分析(overheat_guidance)、检修策略(maintenance_strategy)、四管健康报告智能解读(four_tube_health_interpretation)、泄爆分析(leakage_burst_analysis)、**地面沉降报告（subsidence_daily / weekly / monthly / quarterly / yearly）**
**针对综合分析各专项分析，主编排流程包括的提示词模板(以超温分析为例)**：该流程编排相关提示词(综合分析流程编排相关模板) + NL2SQL 数据计划模板(下面④中analysis_plan_overheat_guidance)

> 看图诊断是基于通用综合分析，然后前置增加了 视觉臂/RAG臂/NL2SQL臂的并行处理，其中NL2SQL臂既是通用综合分析的主链路
...
| 范围 | `app/api/analysis.py`、`app/services/analysis_service.py`、`app/services/analysis_img_diag_upload.py`、`app/llm/graphs/analysis_graph_runner.py`、**`app/llm/graphs/analysis_img_diag_runner.py`**、`app/llm/graphs/analysis_graph_state.py`、`app/models/analysis_nl2sql_llm.py`、`app/services/analysis_trace_store.py`、`app/models/analysis.py`、`app/core/config.py`、`app/core/metrics.py`。 |
...
结论：**核心链路已实现，且 Phase 3 产品化能力已基本落地；编排层已使用 LangGraph `StateGraph`（两套图：payload / nl2sql），并另有看图诊断 **`img_diag`** 第三入口（**并行编排**，见下文）。**

已对齐项：
1. 运行入口：`/analysis/run-with-payload`、`/analysis/run-with-nl2sql`、**`/analysis/run-with-nl2sql-stream`**（NL2SQL 流式 synthesis）、**`/analysis/run-img-diag`**、**`/analysis/run-img-diag-stream`**（看图诊断流式 synthesis，事件形态对齐 nl2sql-stream）（及 **`/analysis/img-diag/upload`**）已落地。
2. **编排内核**：**payload / nl2sql** 在 `AnalysisGraphRunner` 内 **`langgraph.StateGraph`** 编译执行；`langgraph` 不可用时 **顺序回退** 同一套异步节点，并在 `execution_summary.graph_orchestrator` 中区分。**看图诊断** 由 **`AnalysisImgDiagGraphRunner`**：三路 **`asyncio.gather`** 并行后再合成——同步 **`run_with_img_diag`**（**`_generate_summary`**）；流式 **`iter_img_diag_stream_events`**（**`_stream_summary_text`** + 后台 **`_finalize_img_diag_v2`** / trace / 日志）（**未编译为第三套 StateGraph**）。
3. **NL2SQL 计划**：`plan_context_rag` → 可选 **`intent_llm` / `plan_llm`**（`ANALYSIS_NL2SQL_LLM_PLANNER_ENABLED`）→ 与 `analysis_plan_*` 模板 **合并** → **`acquire_data`**（**`_execute_data_plan`**：**默认同 dependency 层并行** `NL2SQLService.query`，见 **`ANALYSIS_NL2SQL_ACQUIRE_*`**）；依赖、重试、跳过与 trace 已落地。
...
9. **看图诊断**：`analysis_type` / **`data_coverage.mode`** / trace **`data_mode`** 均为 **`img_diag`**；**`evidence.vision_findings`**、**`parallel_lane_trace`**；提示词 **`analysis_plan_img_diag`**、**`analysis_img_diag_vision`** 等；详情见 **`企业级综合分析-看图诊断实现和使用说明.md`**。
...
| 分层 | 主要文件 | 关键职责 |
|---|---|---|
| API | `app/api/analysis.py` | **运行入口**：payload / nl2sql / **nl2sql-stream** / **img_diag 同步** / **img_diag-stream**；**`img-diag/upload`**；trace 查询/列表/统计/趋势/TopN。 |
| 服务层 | `app/services/analysis_service.py` | **`run_analysis_*`**（含 **`run_analysis_nl2sql_stream`**、**`run_analysis_img_diag_stream`**）、默认值注入、调用 **`AnalysisImgDiagGraphRunner`**、trace 持久化与检索、统计聚合与监控打点。 |
| 编排内核 | `app/llm/graphs/analysis_graph_runner.py`（**`AnalysisGraphRunner`**）；`app/llm/graphs/analysis_img_diag_runner.py`（**`AnalysisImgDiagGraphRunner`**，继承前者） | **通用基座**：payload / nl2sql 两套编译 **`StateGraph`**、检查点、顺序回退与节点实现；含 NL2SQL 规划前 RAG、可选意图/计划 LLM、计划合并、**`acquire_data`（`_execute_data_plan`：默认同 dependency 层并行 `NL2SQLService.query`）**、质量门、业务 RAG、`synthesis`、`finalize`；**`iter_nl2sql_stream_events`**（NL2SQL 流式）。**看图诊断**：子类 **`asyncio.gather`** 并行「视觉 ‖ NL2SQL 臂（**直接复用父类** `_lg_nl2sql_*` 直至 **`data_quality_gate`**）‖ 业务 RAG」，再 **`run_with_img_diag`**（**`_generate_summary`**）或 **`iter_img_diag_stream_events`**（**`_stream_summary_text`**）；**`_substitute_img_diag_placeholders`**、**`_gather_img_diag_pack`** / **`_finalize_img_diag_v2`**。 |
...
flowchart TB
  subgraph S_HTTP["HTTP 与鉴权"]
    API["POST run-with-payload\n或 run-with-nl2sql /\nrun-with-nl2sql-stream\n或 run-img-diag /\nrun-img-diag-stream\n请求头 Bearer SERVICE_API_KEY"]
  end
  subgraph S_SVC["应用服务层"]
    SVC["类 AnalysisService\nrun_analysis_payload /\nrun_analysis_nl2sql /\nrun_analysis_nl2sql_stream /\nrun_analysis_img_diag /\nrun_analysis_img_diag_stream"]
    DEF["请求参数规范化 AnalysisService._apply_defaults_*\n合并 AnalysisConfig 默认 options"]
    GR["类 AnalysisImgDiagGraphRunner\npayload/nl2sql: LangGraph\nimg_diag: asyncio.gather\nrun_with_img_diag /\niter_img_diag_stream_events"]
  end
  subgraph S_RUN["编排内核"]
...
**看图诊断（`img_diag`）**：**`run-img-diag`** → **`run_analysis_img_diag`** → **`run_with_img_diag`**；**`run-img-diag-stream`** → **`run_analysis_img_diag_stream`** → **`iter_img_diag_stream_events`**（trace 在流结束后异步 **`_save_trace`**）。顶层 **fan-out / join**、RAG 双通道边界与 Trace 字段说明见 **`enterprise-level_transformation_docs/企业级综合分析-看图诊断实现和使用说明.md`**（含详细 Mermaid）。  
说明：图中 **`BR` / `LG` / `SEQ`** 分支刻画 **`run-with-payload` / `run-with-nl2sql`** 是否走编译 LangGraph；**`run-img-diag` / `run-img-diag-stream` 外层并行编排不经该菱形分支**，NL2SQL 臂内部仍复用父类节点函数。
...
### 4.2 NL2SQL 模式：计划任务调度（依赖分层、默认同层并行）

实现见 **`AnalysisGraphRunner._execute_data_plan`**（关闭并行时走 **`_execute_data_plan_sequential`**，语义不变仅调度顺序不同）。

- **分层**：仅当某任务的 **`dependency_ids` 对应前置任务均已 **`success`** 且前置不在「未完成集合」中时，该任务进入本轮 **Runnable**；依赖失败或未满足则最终 **`skipped`**。
- **默认并行**： Runnable 内多条任务在同一轮以 **`asyncio.gather`** 并发调用 **`NL2SQLService.query`**，并由 **`ANALYSIS_NL2SQL_ACQUIRE_MAX_PARALLEL`**（Semaphore）限流；环境变量 **`ANALYSIS_NL2SQL_ACQUIRE_PARALLEL_ENABLED=false`** 时整段退回串行。
- **单任务内部**：每次 **`query`** 仍是「Chain 生成 SQL →（可选 refine）→ **execute**」**串行**，未拆成跨任务并行。

```mermaid
flowchart LR
    P["已定稿的 plan_tasks"] --> W["按 dependency_ids\n分层调度"]
    W --> G["同层 Runnable\nasyncio.gather\n并行 query"]
    G --> K{"依赖项\n是否已成功?"}
    K -->|否| SKIP["标记 skipped\n写入 trace"]
    K -->|是| TRY["单任务 NL2SQL.query\n最多 2 次尝试"]
    TRY -->|成功| OK["落库行数据\n成功 trace"]
...
- **谁构造请求**：`AnalysisGraphRunner`（`app/llm/graphs/analysis_graph_runner.py`）在 **`_execute_data_plan`** 中为每个计划项构造 **`NL2SQLQueryRequest(..., question=task.question, time_intent_text=(req.query).strip())`**，并调用 **`await self._nl2sql.query(..., record_conversation=False, include_parsed_intent=…)`**（避免与分析会话落库重复）。  
- **问句意图**：链内 **`resolve_question_intent`** 解析时间（程序规则）与范围（默认 rule）；`time_intent_text` 用用户原句，避免 plan 长问句 RAG 附录污染；trace 可选 **`question_intent`**（`NL2SQL_TRACE_INCLUDE_QUESTION_INTENT`）。详设 **`docs/NL2SQL自然语言时间和范围窗口解析&改写改造落地方案.md`**。  
- **调度方式**：默认对 **同一依赖层** 的多个 Runnable 任务 **`asyncio.gather` 并行**发起上述 **`query`**（由 **`ANALYSIS_NL2SQL_ACQUIRE_PARALLEL_ENABLED`**、**`ANALYSIS_NL2SQL_ACQUIRE_MAX_PARALLEL`** 控制）；**每条任务内部** 仍为单次 **`NL2SQLService.query`** 的串行闭环。  
...
- **文档交叉**：NL2SQL 接入形态总览见 **`enterprise-level_transformation_docs/企业级NL2SQL基座实现方案.md`** §2、§4。
...
- 语义分析类模板：配置相关业务逻辑，也就是预制系统提示词(流程编排相关模板)
...
#### 5.4.0 当前综合分析prompts模板的读取逻辑如下(以超温分析为例说明)：
	1. 分析流程编排相关模板读取优先级顺序如下：
		...
		analysis_type目前包括：overheat_guidance(超温分析)、maintenance_strategy(检修策略)、four_tube_health_interpretation、leakage_burst_analysis、以及地降 **subsidence_daily|weekly|monthly|quarterly|yearly**（部署须 `NL2SQL_BUSINESS_DOMAIN=subsidence`）。

	2. 数据计划(NL2SQL查询数据)相关模板读取优先级顺序如下(超温分析为例)：
		-- 直接按 scene = "analysis_plan_<analysis_type>" 查
		-- 若不存在或 JSON 非法，才回退到代码内置默认任务（_build_data_plan）

#### 5.4.1 地面沉降专项（`subsidence_*`）

| analysis_type | 典型周期 | 数据计划 scene | 编排模板前缀 |
|---------------|----------|----------------|--------------|
| `subsidence_daily` | 日 | `analysis_plan_subsidence_daily`（约 q1～q3） | `analysis_intent/data_plan/synthesis/report_subsidence_daily` |
| `subsidence_weekly` | 周 | `analysis_plan_subsidence_weekly` | `…_subsidence_weekly` |
| `subsidence_monthly` | 月 | `analysis_plan_subsidence_monthly` | `…_subsidence_monthly` |
| `subsidence_quarterly` | 季 | `analysis_plan_subsidence_quarterly`（约 q1～q4） | `…_subsidence_quarterly` |
| `subsidence_yearly` | 年 | `analysis_plan_subsidence_yearly`（约 q1～q5） | `…_subsidence_yearly` |

**前置**：`.env` 设 `NL2SQL_BUSINESS_DOMAIN=subsidence` + `DB_PASSWORD`（PG）；摄入 `configs/nl2sql_business/subsidence/rag/` 三 namespace；镜像含 **asyncpg**。取数仍走同一 `_execute_data_plan` → `NL2SQLService.query`（语义链接 / PG 方言由基座配置包生效）。
...
```json
{
  ...
  "query": "请生成本季度地面沉降监测分析报告",
  "user_id": "ops",
  "session_id": "subsidence-q-demo"
}
```

详设：`docs/基于地降所项目改造/NL2SQL基座改造.md`；基座链路：`企业级NL2SQL基座实现方案.md`。
...
4. **v2 多槽 synthesis（超温）**：**不**使用 `analysis_synthesis_overheat_guidance` v2 作为 LLM system；使用 `analysis_synthesis_overheat_narrative` + 槽位 `narrative_instruction`（见 `analysis_synthesis_v2.py`）。`analysis_synthesis_overheat_guidance` v2 仍为九段报告结构蓝本，供 v1 路径与文档对齐。

---

#### 5.4.3 大类 Ⅱ-a：NL2SQL「多步取数」数据计划（JSON，不是 SQL 正文）

- **顶级键**：**`analysis_plan_<analysis_type>`**；锅炉侧常见 `overheat_guidance` | `maintenance_strategy` | `custom` 等；地降侧为 **`subsidence_daily|weekly|monthly|quarterly|yearly`**（见 §5.4.1）。  
...
- **与综合分析编排的关系**：综合分析图里 **`acquire_data`** 会多次调用 NL2SQL 服务（**`_execute_data_plan`**：**默认同 dependency 层并行** `query`，见 **`ANALYSIS_NL2SQL_ACQUIRE_*`**），每次仍走该主链路提示词；但**不要**把 **`analysis_plan_*`** 与 **`nl2sql`** 混成同一种模板。  
...
| `ANALYSIS_NL2SQL_ACQUIRE_PARALLEL_ENABLED` | `true` | `acquire_data` 同 dependency 层是否并行 `NL2SQLService.query` |
| `ANALYSIS_NL2SQL_ACQUIRE_MAX_PARALLEL` | `8` | 同层并行上限（Semaphore） |
| `ANALYSIS_SYNTHESIS_STRATEGY` | `v1` | 全局 synthesis 策略：`v1` 单次 LLM；`v2` 多槽位（见 `docs/综合分析优化版本实现方案(v2版本).md`） |
| `ANALYSIS_SYNTHESIS_STRATEGY_<专项>` | 空 | 按 `analysis_type` 覆盖全局，如 `..._OVERHEAT_GUIDANCE=v2`；无槽位注册表时 effective 回退 v1 |
| `ANALYSIS_PLAN_TEMPLATE_VERSION` / `ANALYSIS_SYNTHESIS_TEMPLATE_VERSION` | 空 | 全局 plan/synthesis 模板 version；专项可用 `_*_<专项>` 覆盖 |
| `ANALYSIS_SYNTHESIS_V2_*` | 见 `.env.example` | v2 并行 LLM 数、单槽 max_tokens、表行数上限、流式首槽真流式等 |
| `ANALYSIS_SYNTHESIS_TIMEOUT_SECONDS` | `90` | synthesis 超时（秒）；v1/v2 共用 |
| `ANALYSIS_SYNTHESIS_GATHERED_JSON_MAX_CHARS` | `16000` | v1 user / v2 各槽 user 中 `gathered_data` JSON 截断上限 |
| `ANALYSIS_SYNTHESIS_MAX_TOKENS` | `8192` | **v1** 整篇输出 `max_tokens`；v2 单槽见 `ANALYSIS_SYNTHESIS_V2_SEGMENT_MAX_TOKENS` |
...
当前综合分析实现已经具备企业级核心能力（**payload / nl2sql 双 LangGraph + 看图诊断并行编排**、可选检查点、NL2SQL 多阶段计划、审计、监控、持久化、回放），可进入生产试运行阶段。  
