...
> 当前通用综合分析，支持的专项分析类型：超温分析(overheat_guidance)、检修策略(maintenance_strategy)、四管健康报告智能解读(four_tube_health_interpretation)、泄爆分析(leakage_burst_analysis)
...
#### 5.4.0 当前综合分析prompts模板的读取逻辑如下(以超温分析为例说明)：
	1. 分析流程编排相关模板读取优先级顺序如下：
		...
		analysis_type目前包括：overheat_guidance(超温分析)、maintenance_strategy(检修策略)
