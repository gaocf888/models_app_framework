...
基础镜像需允许在构建阶段执行包安装与 `pip`。**宿主机麒麟与容器底座 OS 无关**：英伟达常用 `Dockerfile-nvidia`（容器内 apt）；沐曦/寒武纪常用 `Dockerfile-mx`（Metax 等底座多为 Kylin/RHEL 系 → yum/dnf）；昇腾用 `Dockerfile-ascend`（官方 `vllm-ascend` 多为 Ubuntu 系 → apt）。由平台 overlay 自动选择 Dockerfile。
...
默认离线路径约定：宿主机 `MODEL_PATH=/aidata/models/llm`，并通过 Compose 挂载到容器 `/workspace/models`。
...
python -c "from modelscope import snapshot_download; snapshot_download('qwen/Qwen2.5-VL-7B-Instruct', cache_dir='/aidata/models/llm/Qwen2.5-VL-7B-Instruct')"
```

```text
# 方式三：Hugging Face（可配合镜像站）
pip install huggingface-hub
python -c "from huggingface_hub import snapshot_download; snapshot_download('Qwen/Qwen2.5-VL-7B-Instruct', local_dir='/aidata/models/llm/Qwen2.5-VL-7B-Instruct', endpoint='https://hf-mirror.com')"
```

在 `config/vllm.yaml` / `config/models.yaml` 中确认模型路径与预设一致；若使用 `MODEL_PATH` 环境变量（默认 `/aidata/models/llm`）挂载自定义目录，请与配置中的路径对应。
...
3. **选择平台（自动匹配 Dockerfile）**
   - `docker/` 下 Dockerfile：`Dockerfile-nvidia`（英伟达 / apt）、`Dockerfile-mx`（沐曦·寒武纪 / yum）、`Dockerfile-ascend`（昇腾 / apt）。
